...
class GL12 extends GL11
{
    ...
    /**
     * {@see GL46::glDrawRangeElements} is a restricted form of
     * {@see GL46::glDrawElements}. $mode, and $count match the corresponding
     * arguments to {@see GL46::glDrawElements}, with the additional
     ...
     * data, which may be queried by calling {@see GL46::glGet} with argument
     * {@see GL46::GL_MAX_ELEMENTS_VERTICES} and
     * {@see GL46::GL_MAX_ELEMENTS_INDICES}. If    end - start + 1   is
     * greater than the value of {@see GL46::GL_MAX_ELEMENTS_VERTICES}, or if
     * $count is greater than the value of
     * {@see GL46::GL_MAX_ELEMENTS_INDICES}, then the call may operate at
     ...
     *
     * When {@see GL46::glDrawRangeElements} is called, it uses $count
     ...
     * {@see GL46::glDrawRangeElements} have an unspecified value after
     * {@see GL46::glDrawRangeElements} returns. Attributes that aren't
     ...
     * @return void
     */
    public function glDrawRangeElements($mode, $start, $end, $count, $type, ?\FFI\CData $indices): void
    {
     ...
     * disable three-dimensional texturing, call {@see GL46::glEnable} and
     * {@see GL46::glDisable} with argument {@see GL46::GL_TEXTURE_3D}.
     *
     * To define texture images, call {@see GL46::glTexImage3D}. The
     * arguments describe the parameters of the texture image, such as
     * height, width, depth, width of the border, level-of-detail number (see
     * {@see GL46::glTexParameter}), and number of color components provided.
     * The last three arguments describe how the image is represented in
     * memory.
     *
     * If $target is {@see GL46::GL_PROXY_TEXTURE_3D}, no data is read from
     ...
     *
     * If $target is {@see GL46::GL_TEXTURE_3D}, data is read from $data as a
     ...
     * depending on $format, to form elements. Each data byte is treated as
     * eight 1-bit elements, with bit ordering determined by
     * {@see GL46::GL_UNPACK_LSB_FIRST} (see {@see GL46::glPixelStore}).
     *
     ...
     * match exactly. (The representations specified by {@see GL46::GL_RED},
     * {@see GL46::GL_RG}, {@see GL46::GL_RGB}, and {@see GL46::GL_RGBA} must
     * match exactly.)
     *
     ...
     * formats, {@see GL46::GL_COMPRESSED_RED},
     * {@see GL46::GL_COMPRESSED_RG}, {@see GL46::GL_COMPRESSED_RGB}, or
     * {@see GL46::GL_COMPRESSED_RGBA}, the GL will replace the internal
     * format with the symbolic constant for a specific internal format and
     * compress the texture before storage. If no corresponding internal
     * format is available, or the GL can not compress that image for any
     * reason, the internal format is instead replaced with a corresponding
     * base internal format.
     *
     * If the $internalformat parameter is {@see GL46::GL_SRGB},
     * {@see GL46::GL_SRGB8}, {@see GL46::GL_SRGB_ALPHA}, or
     * {@see GL46::GL_SRGB8_ALPHA8}, the texture is treated as if the red,
     ...
     * sRGB encoded component  c s   to a linear component  c l   is:
     *
     ...
     * its best match for the requested storage resolution and format. To
     * then query this state, call {@see GL46::glGetTexLevelParameter}. If
     ...
     * @return void
     */
    public function glTexImage3D($target, $level, $internalformat, $width, $height, $depth, $border, $format, $type, ?\FFI\CData $pixels): void
    {
        ...
        assert(Assert::int16($internalformat), 'Argument $internalformat must be a C-like GLint, but incompatible or overflow value given');
     ...
     * @return void
     */
    public function glTexSubImage3D($target, $level, $xoffset, $yoffset, $zoffset, $width, $height, $depth, $format, $type, ?\FFI\CData $pixels): void
    {
        $target = $target instanceof \FFI\CData ? $target->cdata : $target;
        $level = $level instanceof \FFI\CData ? $level->cdata : $level;
     ...
     * @return void
     */
    public function glCopyTexSubImage3D($target, $level, $xoffset, $yoffset, $zoffset, $x, $y, $width, $height): void
    {
        $target = $target instanceof \FFI\CData ? $target->cdata : $target;
        $level = $level instanceof \FFI\CData ? $level->cdata : $level;
        $xoffset = $xoffset instanceof \FFI\CData ? $xoffset->cdata : $xoffset;
