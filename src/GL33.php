...
class GL33 extends GL32
{
    ...
    /**
     * {@see GL46::glBindFragDataLocationIndexed} specifies that the varying
     * out variable $name in $program should be bound to fragment color
     * $colorNumber when the program is next linked. $index may be zero or
     * one to specify that the color be used as either the first or second
     * color input to the blend equation, respectively.
     *
     * The bindings specified by {@see GL46::glBindFragDataLocationIndexed}
     * have no effect until $program is next linked. Bindings may be
     * specified at any time after $program has been created. Specifically,
     * they may be specified before shader objects are attached to the
     * program. Therefore, any name may be specified in $name, including a
     * name that is never used as a varying out variable in any fragment
     * shader object. Names beginning with {@see GL46::gl_} are reserved by
     * the GL.
     *
     * If $name was bound previously, its assigned binding is replaced with
     * $colorNumber and $index. $name must be a null-terminated string.
     * $index must be less than or equal to one, and $colorNumber must be
     * less than the value of {@see GL46::GL_MAX_DRAW_BUFFERS} if $index is
     * zero, and less than the value of
     * {@see GL46::GL_MAX_DUAL_SOURCE_DRAW_BUFFERS} if index is greater than
     * or equal to one.
     *
     * In addition to the errors generated by
     * {@see GL46::glBindFragDataLocationIndexed}, the program $program will
     * fail to link if:    The number of active outputs is greater than the
     * value {@see GL46::GL_MAX_DRAW_BUFFERS}.     More than one varying out
     ...
     * @return void
     */
    public function glBindFragDataLocationIndexed($program, $colorNumber, $index, ?\FFI\CData $name): void
    {
        ...
        assert(Assert::uint16($colorNumber), 'Argument $colorNumber must be a C-like GLuint, but incompatible or overflow value given');
    ...
    /**
     * {@see GL46::glGetFragDataIndex} returns the index of the fragment
     * color to which the variable $name was bound when the program object
     * $program was last linked. If $name is not a varying out variable of
     ...
     */
    public function glGetFragDataIndex($program, ?\FFI\CData $name): int
    {
        $program = $program instanceof \FFI\CData ? $program->cdata : $program;
    ...
    /**
     * {@see GL46::glGenSamplers} returns $n sampler object names in
     * $samplers. There is no guarantee that the names form a contiguous set
     * of integers; however, it is guaranteed that none of the returned names
     * was in use immediately before the call to {@see GL46::glGenSamplers}.
     *
     * Sampler object names returned by a call to {@see GL46::glGenSamplers}
     * are not returned by subsequent calls, unless they are first deleted
     * with {@see GL46::glDeleteSamplers}.
     *
     * The names returned in $samplers are marked as used, for the purposes
     * of {@see GL46::glGenSamplers} only, but they acquire state and type
     ...
     * @return void
     */
    public function glGenSamplers($count, ?\FFI\CData $samplers): void
    {
    ...
    /**
     * {@see GL46::glDeleteSamplers} deletes $n sampler objects named by the
     ...
     * to a sampler unit is deleted, it is as though
     * {@see GL46::glBindSampler} is called with unit set to the unit the
     * sampler is bound to and sampler zero. Unused names in samplers are
     ...
     * @return void
     */
    public function glDeleteSamplers($count, ?\FFI\CData $samplers): void
    {
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
    ...
    /**
     * {@see GL46::glIsSampler} returns {@see GL46::GL_TRUE} if $id is
     * currently the name of a sampler object. If $id is zero, or is a
     * non-zero value that is not currently the name of a sampler object, or
     * if an error occurs, {@see GL46::glIsSampler} returns
     * {@see GL46::GL_FALSE}.
     *
     * A name returned by {@see GL46::glGenSamplers}, is the name of a
     * sampler object.
     *
     ...
     * @return int|\FFI\CData|\FFI\CInt
     */
    public function glIsSampler($sampler): int
    {
        $sampler = $sampler instanceof \FFI\CData ? $sampler->cdata : $sampler;

        assert(Assert::uint16($sampler), 'Argument $sampler must be a C-like GLuint, but incompatible or overflow value given');
    ...
    /**
     * {@see GL46::glBindSampler} binds $sampler to the texture unit at index
     * $unit. $sampler must be zero or the name of a sampler object
     * previously returned from a call to {@see GL46::glGenSamplers}. $unit
     * must be less than the value of
     * {@see GL46::GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS}.
     *
     ...
     * @return void
     */
    public function glBindSampler($unit, $sampler): void
    {
    ...
    /**
     * {@see GL46::glSamplerParameter} assigns the value or values in $params
     * to the sampler parameter specified as $pname. $sampler specifies the
     * sampler object to be modified, and must be the name of a sampler
     * object previously returned from a call to {@see GL46::glGenSamplers}.
     ...
     *    four use mipmaps.   A mipmap is an ordered set of arrays representing
     *    the same image at progressively lower resolutions. If the texture has
     *    dimensions    2 n  × 2 m   , there are     max &amp;af;  n m   + 1
     *    mipmaps. The first mipmap is the original texture, with dimensions
     *    2 n  × 2 m   . Each subsequent mipmap has dimensions    2   k - 1
     *    × 2   l - 1     , where    2 k  × 2 l    are the dimensions of the
     *    previous mipmap, until either    k = 0   or    l = 0  . At that point,
     *    subsequent mipmaps have dimension    1 × 2   l - 1      or    2   k -
     *    1    × 1   until the final mipmap, which has dimension    1 × 1  .
     *    To define the mipmaps, call {@see GL46::glTexImage1D},
     *    {@see GL46::glTexImage2D}, {@see GL46::glTexImage3D},
     *    {@see GL46::glCopyTexImage1D}, or {@see GL46::glCopyTexImage2D} with
     *    the level argument indicating the order of the mipmaps. Level 0 is the
     *    original texture; level    max &amp;af;  n m    is the final    1 × 1
     *      mipmap.   $params supplies a function for minifying the texture as
     *    one of the following:   {@see GL46::GL_NEAREST}   Returns the value of
     *    the texture element that is nearest (in Manhattan distance) to the
     *    center of the pixel being textured.     {@see GL46::GL_LINEAR}
     ...
     *    texture value.     {@see GL46::GL_LINEAR_MIPMAP_NEAREST}   Chooses the
     ...
     *    to produce a texture value.     {@see GL46::GL_NEAREST_MIPMAP_LINEAR}
     ...
     *    The initial value of {@see GL46::GL_TEXTURE_MIN_FILTER} is
     *    {@see GL46::GL_NEAREST_MIPMAP_LINEAR}.
     *
     *  - {@see GL46::GL_TEXTURE_MAG_FILTER}: The texture magnification function
     *    is used when the pixel being
     *    textured maps to an area less than or equal to one texture element. It
     *    sets the texture magnification function to either
     *    {@see GL46::GL_NEAREST} or {@see GL46::GL_LINEAR} (see below).
     *    {@see GL46::GL_NEAREST} is generally faster than
     *    {@see GL46::GL_LINEAR}, but it can produce textured images with
     *    sharper edges because the transition between texture elements is not
     *    as smooth. The initial value of {@see GL46::GL_TEXTURE_MAG_FILTER} is
     *    {@see GL46::GL_LINEAR}.   {@see GL46::GL_NEAREST}   Returns the value
     *    of the texture element that is nearest (in Manhattan distance) to the
     *    center of the pixel being textured.     {@see GL46::GL_LINEAR}
     *    Returns the weighted average of the four texture elements that are
     *    closest to the center of the pixel being textured. These can include
     *    border texture elements, depending on the values of
     ...
     *    {@see GL46::GL_CLAMP_TO_EDGE}, {@see GL46::GL_MIRRORED_REPEAT},
     *    {@see GL46::GL_REPEAT}, or {@see GL46::GL_MIRROR_CLAMP_TO_EDGE}.
     *    {@see GL46::GL_CLAMP_TO_BORDER} causes the s coordinate to be clamped
     *    to the range       -1  2N      1 +    1  2N       , where N is the
     *    size of the texture in the direction of
     *    clamping.{@see GL46::GL_CLAMP_TO_EDGE} causes s coordinates to be
     *    clamped to the range       1  2N      1 -    1  2N       , where N is
     *    the size of the texture in the direction of clamping.
     *    {@see GL46::GL_REPEAT} causes the integer part of the s coordinate to
     *    be ignored; the GL uses only the fractional part, thereby creating a
     *    repeating pattern. {@see GL46::GL_MIRRORED_REPEAT} causes the s
     *    coordinate to be set to the fractional part of the texture coordinate
     *    if the integer part of s is even; if the integer part of s is odd,
     *    then the s texture coordinate is set to    1 -  frac &amp;af;  s    ,
     *    where    frac &amp;af;  s    represents the fractional part of s.
     ...
     *    {@see GL46::GL_CLAMP_TO_EDGE}, {@see GL46::GL_MIRRORED_REPEAT},
     *    {@see GL46::GL_REPEAT}, or {@see GL46::GL_MIRROR_CLAMP_TO_EDGE}. See
     *    the discussion under {@see GL46::GL_TEXTURE_WRAP_S}. Initially,
     ...
     *    mode for currently bound textures.
     ...
     *    the discussion of {@see GL46::GL_TEXTURE_COMPARE_FUNC} for details of
     *    how the comparison is evaluated. The result of the comparison is
     ...
     *    the texture value sampled from the currently bound texture. result is
     *    assigned to   R t  .
     *
     ...
     * @return void
     */
    public function glSamplerParameteri($sampler, $pname, $param): void
    {
        $sampler = $sampler instanceof \FFI\CData ? $sampler->cdata : $sampler;
     ...
     * @return void
     */
    public function glSamplerParameteriv($sampler, $pname, ?\FFI\CData $param): void
    {
        $sampler = $sampler instanceof \FFI\CData ? $sampler->cdata : $sampler;
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
     ...
     * @return void
     */
    public function glSamplerParameterf($sampler, $pname, $param): void
    {
        $sampler = $sampler instanceof \FFI\CData ? $sampler->cdata : $sampler;
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
        $param = $param instanceof \FFI\CData ? $param->cdata : $param;
     ...
     * @return void
     */
    public function glSamplerParameterfv($sampler, $pname, ?\FFI\CData $param): void
    {
        $sampler = $sampler instanceof \FFI\CData ? $sampler->cdata : $sampler;
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
     ...
     * @return void
     */
    public function glSamplerParameterIiv($sampler, $pname, ?\FFI\CData $param): void
    {
        $sampler = $sampler instanceof \FFI\CData ? $sampler->cdata : $sampler;
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
     ...
     * @return void
     */
    public function glSamplerParameterIuiv($sampler, $pname, ?\FFI\CData $param): void
    {
        $sampler = $sampler instanceof \FFI\CData ? $sampler->cdata : $sampler;
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
    ...
    /**
     * {@see GL46::glGetSamplerParameter} returns in $params the value or
     * values of the sampler parameter specified as $pname. $sampler defines
     * the target sampler, and must be the name of an existing sampler
     * object, returned from a previous call to {@see GL46::glGenSamplers}.
     * $pname accepts the same symbols as {@see GL46::glSamplerParameter},
     ...
     * @return void
     */
    public function glGetSamplerParameteriv($sampler, $pname, ?\FFI\CData $params): void
    {
        $sampler = $sampler instanceof \FFI\CData ? $sampler->cdata : $sampler;
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
     ...
     * @return void
     */
    public function glGetSamplerParameterIiv($sampler, $pname, ?\FFI\CData $params): void
    {
        $sampler = $sampler instanceof \FFI\CData ? $sampler->cdata : $sampler;
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
     ...
     * @return void
     */
    public function glGetSamplerParameterfv($sampler, $pname, ?\FFI\CData $params): void
    {
        $sampler = $sampler instanceof \FFI\CData ? $sampler->cdata : $sampler;
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
     ...
     * @return void
     */
    public function glGetSamplerParameterIuiv($sampler, $pname, ?\FFI\CData $params): void
    {
        $sampler = $sampler instanceof \FFI\CData ? $sampler->cdata : $sampler;
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
    ...
    /**
     * {@see GL46::glQueryCounter} causes the GL to record the current time
     * into the query object named $id. $target must be
     * {@see GL46::GL_TIMESTAMP}. The time is recorded after all previous
     * commands on the GL client and server state and the framebuffer have
     * been fully realized. When the time is recorded, the query result for
     * that object is marked available. {@see GL46::glQueryCounter} timer
     * queries can be used within a {@see GL46::glBeginQuery} /
     * {@see GL46::glEndQuery} block where the target is
     * {@see GL46::GL_TIME_ELAPSED} and it does not affect the result of that
     * query object.
     *
     ...
     * @return void
     */
    public function glQueryCounter($id, $target): void
    {
     ...
     * @return void
     */
    public function glGetQueryObjecti64v($id, $pname, ?\FFI\CData $params): void
    {
        $id = $id instanceof \FFI\CData ? $id->cdata : $id;
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
     ...
     * @return void
     */
    public function glGetQueryObjectui64v($id, $pname, ?\FFI\CData $params): void
    {
        $id = $id instanceof \FFI\CData ? $id->cdata : $id;
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
    ...
    /**
     * {@see GL46::glVertexAttribDivisor} modifies the rate at which generic
     ...
     * at slot $index advances once per vertex. If $divisor is non-zero, the
     * attribute advances once per $divisor instances of the set(s) of
     * vertices being rendered. An attribute is referred to as instanced if
     * its {@see GL46::GL_VERTEX_ATTRIB_ARRAY_DIVISOR} value is non-zero.
     *
     ...
     * @return void
     */
    public function glVertexAttribDivisor($index, $divisor): void
    {
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
     ...
     * @return void
     */
    public function glVertexAttribP1ui($index, $type, $normalized, $value): void
    {
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
        ...
        assert(Assert::uint16($value), 'Argument $value must be a C-like GLuint, but incompatible or overflow value given');
     ...
     * @return void
     */
    public function glVertexAttribP1uiv($index, $type, $normalized, ?\FFI\CData $value): void
    {
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
        $type = $type instanceof \FFI\CData ? $type->cdata : $type;
        $normalized = $normalized instanceof \FFI\CData ? $normalized->cdata : $normalized;
     ...
     * @return void
     */
    public function glVertexAttribP2ui($index, $type, $normalized, $value): void
    {
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
        $type = $type instanceof \FFI\CData ? $type->cdata : $type;
        $normalized = $normalized instanceof \FFI\CData ? $normalized->cdata : $normalized;
     ...
     * @return void
     */
    public function glVertexAttribP2uiv($index, $type, $normalized, ?\FFI\CData $value): void
    {
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
        $type = $type instanceof \FFI\CData ? $type->cdata : $type;
        $normalized = $normalized instanceof \FFI\CData ? $normalized->cdata : $normalized;
     ...
     * @return void
     */
    public function glVertexAttribP3ui($index, $type, $normalized, $value): void
    {
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
        $type = $type instanceof \FFI\CData ? $type->cdata : $type;
        $normalized = $normalized instanceof \FFI\CData ? $normalized->cdata : $normalized;
     ...
     * @return void
     */
    public function glVertexAttribP3uiv($index, $type, $normalized, ?\FFI\CData $value): void
    {
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
        $type = $type instanceof \FFI\CData ? $type->cdata : $type;
        $normalized = $normalized instanceof \FFI\CData ? $normalized->cdata : $normalized;
     ...
     * @return void
     */
    public function glVertexAttribP4ui($index, $type, $normalized, $value): void
    {
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
        $type = $type instanceof \FFI\CData ? $type->cdata : $type;
        $normalized = $normalized instanceof \FFI\CData ? $normalized->cdata : $normalized;
     ...
     * @return void
     */
    public function glVertexAttribP4uiv($index, $type, $normalized, ?\FFI\CData $value): void
    {
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
        $type = $type instanceof \FFI\CData ? $type->cdata : $type;
        $normalized = $normalized instanceof \FFI\CData ? $normalized->cdata : $normalized;
