...
class GL45 extends GL44
{
    ...
    /**
     * {@see GL46::glClipControl} controls the clipping volume behavior and
     ...
     * $z_{min} = -w_c$ when $depth is {@see GL46::GL_NEGATIVE_ONE_TO_ONE},
     * and $z_{min} = 0$ when $depth is {@see GL46::GL_ZERO_TO_ONE}.
     *
     * The normalized device coordinate $y_d$ is given by $$y_d = { { f
     * \\times y_c } \\over w_c }$$ where $f = 1$ when $origin is
     * {@see GL46::GL_LOWER_LEFT}, and $f = -1$ when $origin is
     * {@see GL46::GL_UPPER_LEFT}.
     *
     * The window coordinate $z_w$ is given by $$z_w = s \\times z_d + b$$
     * where $s = { { f - n } \\over 2 }$ and $b = { {n + f} \\over 2 }$ when
     * $depth is {@see GL46::GL_NEGATIVE_ONE_TO_ONE}, and $s = f - n$ and $b
     * = n$ when $depth is {@see GL46::GL_ZERO_TO_ONE}. $n$ and $f$ are the
     * near and far depth range values set with {@see GL46::glDepthRange}.
     *
     * Finally, the polygon area computation defined by
     * {@see GL46::gl_FrontFacing} to determine if a polygon is front- or
     * back-facing has its sign negated when $origin is
     * {@see GL46::GL_UPPER_LEFT}.
     *
    ...
    /**
     * {@see GL46::glCreateTransformFeedbacks} returns $n previously unused
    ...
    /**
     * {@see GL46::glTransformFeedbackBufferBase} binds the buffer object
     * $buffer to the binding point at index $index of the transform feedback
     * object $xfb.
     *
    ...
    /**
     * {@see GL46::glTransformFeedbackBufferRange} binds a range of the
     * buffer object $buffer represented by $offset and $size to the binding
     ...
     * achieved by either using {@see GL46::glBindBufferBase} or
     * {@see GL46::glBindBuffersBase} to associate whole buffer object
     * storage to one of the Transform Feedback Binding Points, or by calling
     * {@see GL46::glBindBufferRange} or {@see GL46::glBindBuffersRange} to
     * use a region of a buffer object storage for the binding. You may want
     * to (but are not required to) bind a Transform Feedback Object first,
     * in order to cache the binding configuration. This usually allows you
     * to restore the Transform Feedback configuration faster, than if you
     ...
     * and includes {@see GL46::glGetTransformFeedbackiv} function.
     * {@see GL46::glGetTransformFeedbackiv} can be used to retrieve
     * information about Transform Feedback object bound to the general
     * Transform Feedback binding point, as configured with a
     * {@see GL46::glBindTransformFeedback} call. In this case, you can
     * check:
     *
     *   What the ID of the currently bound Transform Feedback Object is;
     * ({@see GL46::GL_TRANSFORM_FEEDBACK_BUFFER_BINDING})   Whether the
     * Transform Feedback process is currently paused;
     * ({@see GL46::GL_TRANSFORM_FEEDBACK_PAUSED})   Whether the Transform
     * Feedback process has been begun and is currently undergoing;
     * ({@see GL46::GL_TRANSFORM_FEEDBACK_ACTIVE})
     *
     * The latter class, which includes
     * {@see GL46::glGetTransformFeedbacki_v} and
     * {@see GL46::glGetTransformFeedbacki64_v} functions, can be used to
     * check what the current configuration of each of the buffer object
     * regions bound to Transform Feedback Buffer binding points is. This
     * allows you to query for the following information:
     *
     *   {@see GL46::glGetTransformFeedbacki_v} only: What the ID of the
     * Buffer Object bound to a Transform Feedback Binding Point of
     * user-specified index is;
     * ({@see GL46::GL_TRANSFORM_FEEDBACK_BUFFER_BINDING}).
     * {@see GL46::glGetTransformFeedbacki64_v} only: What the start offset
     * configured for the binding is;
     * ({@see GL46::GL_TRANSFORM_FEEDBACK_BUFFER_START});
     * {@see GL46::glGetTransformFeedbacki64_v} only: What the length of the
     * region used for the binding is;
     * ({@see GL46::GL_TRANSFORM_FEEDBACK_BUFFER_SIZE});
     *
    ...
    /**
     * {@see GL46::glCreateBuffers} returns $n previously unused buffer names
     * in $buffers, each representing a new buffer object initialized as if
    ...
    /**
     * {@see GL46::glBufferStorage} and {@see GL46::glNamedBufferStorage}
     * create a new immutable data store. For {@see GL46::glBufferStorage},
     * the buffer object currently bound to $target will be initialized. For
     * {@see GL46::glNamedBufferStorage}, $buffer is the name of the buffer
     * object that will be configured. The size of the data store is
     ...
     * store, $data should be {@see GL46::NULL}.
     *
     * The $flags parameters specifies the intended usage of the buffer's
     * data store. It must be a bitwise combination of a subset of the
     * following flags:   {@see GL46::GL_DYNAMIC_STORAGE_BIT}  The contents
     * of the data store may be updated after creation through calls to
     * {@see GL46::glBufferSubData}. If this bit is not set, the buffer
     * content may not be directly updated by the client. The data argument
     * may be used to specify the initial content of the buffer's data store
     * regardless of the presence of the {@see GL46::GL_DYNAMIC_STORAGE_BIT}.
     * Regardless of the presence of this bit, buffers may always be updated
     * with server-side calls such as {@see GL46::glCopyBufferSubData} and
     * {@see GL46::glClearBufferSubData}.     {@see GL46::GL_MAP_READ_BIT}
     ...
     *   {@see GL46::GL_MAP_WRITE_BIT}  The data store may be mapped by the
     * client for write access and a pointer in the client's address space
     * obtained that may be written through.
     * {@see GL46::GL_MAP_PERSISTENT_BIT}  The client may request that the
     * server read from or write to the buffer while it is mapped. The
     * client's pointer to the data store remains valid so long as the data
     * store is mapped, even during execution of drawing or dispatch
     * commands.    {@see GL46::GL_MAP_COHERENT_BIT}  Shared access to
     * buffers that are simultaneously mapped for client access and are used
     * by the server will be coherent, so long as that mapping is performed
     * using {@see GL46::glMapBufferRange}. That is, data written to the
     * store by either the client or server will be immediately visible to
     * the other with no further action taken by the application. In
     * particular,   If {@see GL46::GL_MAP_COHERENT_BIT} is not set and the
     * client performs a write followed by a call to the
     * {@see GL46::glMemoryBarrier} command with the
     * {@see GL46::GL_CLIENT_MAPPED_BUFFER_BARRIER_BIT} set, then in
     * subsequent commands the server will see the writes.   If
     * {@see GL46::GL_MAP_COHERENT_BIT} is set and the client performs a
     * write, then in subsequent commands the server will see the writes.
     * If {@see GL46::GL_MAP_COHERENT_BIT} is not set and the server performs
     * a write, the application must call {@see GL46::glMemoryBarrier} with
     * the {@see GL46::GL_CLIENT_MAPPED_BUFFER_BARRIER_BIT} set and then call
     * {@see GL46::glFenceSync} with
     * {@see GL46::GL_SYNC_GPU_COMMANDS_COMPLETE} (or {@see GL46::glFinish}).
     * Then the CPU will see the writes after the sync is complete.   If
     * {@see GL46::GL_MAP_COHERENT_BIT} is set and the server does a write,
     * the app must call {@see GL46::glFenceSync} with
     * {@see GL46::GL_SYNC_GPU_COMMANDS_COMPLETE} (or {@see GL46::glFinish}).
     * Then the CPU will see the writes after the sync is complete.
     * {@see GL46::GL_CLIENT_STORAGE_BIT}  When all other criteria for the
     * buffer storage allocation are met, this bit may be used by an
     * implementation to determine whether to use storage that is local to
     * the server or to the client to serve as the backing store for the
     * buffer.
     *
     * The allowed combinations of flags are subject to certain restrictions.
     * They are as follows:   If $flags contains
     * {@see GL46::GL_MAP_PERSISTENT_BIT}, it must also contain at least one
     * of {@see GL46::GL_MAP_READ_BIT} or {@see GL46::GL_MAP_WRITE_BIT}.   If
     * $flags contains {@see GL46::GL_MAP_COHERENT_BIT}, it must also contain
    ...
    /**
     * {@see GL46::glClearBufferData} and {@see GL46::glClearNamedBufferData}
     * fill the entirety of a buffer object's data store with data from
     * client memory.
     *
     * Data, initially supplied in a format specified by $format in data type
     * $type is read from the memory address given by $data and converted
     * into the internal representation given by $internalformat, which must
     * be one of the following sized internal formats:
     *
    ...
    /**
     * {@see GL46::glGetBufferSubData} and
     * {@see GL46::glGetNamedBufferSubData} return some or all of the data
     * contents of the data store of the specified buffer object. Data
     * starting at byte offset $offset and extending for $size bytes is
     * copied from the buffer object's data store to the memory pointed to by
     * $data. An error is thrown if the buffer object is currently mapped, or
     * if $offset and $size together define a range beyond the bounds of the
    ...
    /**
     * {@see GL46::glClearBufferSubData} and
     * {@see GL46::glClearNamedBufferSubData} fill a specified region of a
    ...
    /**
     * {@see GL46::glMapBuffer} and {@see GL46::glMapNamedBuffer} map the
     * entire data store of a specified buffer object into the client's
     * address space. The data can then be directly read and/or written
     * relative to the returned pointer, depending on the specified $access
     * policy.
     *
     * A pointer to the beginning of the mapped range is returned once all
     * pending operations on that buffer object have completed, and may be
     * used to modify and/or query the corresponding range of the data store
     * according to the value of $access:    {@see GL46::GL_READ_ONLY}
     * indicates that the returned pointer may be used to read buffer object
     * data.     {@see GL46::GL_WRITE_ONLY} indicates that the returned
     * pointer may be used to modify buffer object data.
     * {@see GL46::GL_READ_WRITE} indicates that the returned pointer may be
     * used to read and to modify buffer object data.
     *
     * If an error is generated, a `NULL` pointer is returned.
     *
     * If no error occurs, the returned pointer will reflect an allocation
     * aligned to the value of {@see GL46::GL_MIN_MAP_BUFFER_ALIGNMENT} basic
     * machine units.
     *
     * The returned pointer values may not be passed as parameter values to
     * GL commands. For example, they may not be used to specify array
     ...
     *
     * No GL error is generated if the returned pointer is accessed in a way
     * inconsistent with $access (e.g. used to read from a mapping made with
     * $access {@see GL46::GL_WRITE_ONLY} or write to a mapping made with
     * $access {@see GL46::GL_READ_ONLY}), but the result is undefined and
     ...
     * performance characteristics. For example, such mappings may be marked
     * as uncacheable regions of memory, and in such cases reading from them
     * may be very slow. To ensure optimal performance, the client should use
     * the mapping in a fashion consistent with the values of
     * {@see GL46::GL_BUFFER_USAGE} for the buffer object and of $access.
    ...
    /**
     * {@see GL46::glMapBufferRange} and {@see GL46::glMapNamedBufferRange}
     * map all or part of the data store of a specified buffer object into
     * the client's address space. $offset and $length indicate the range of
     * data in the buffer object that is to be mapped, in terms of basic
     * machine units. $access is a bitfield containing flags which describe
     * the requested mapping. These flags are described below.
     *
     * A pointer to the beginning of the mapped range is returned once all
     * pending operations on the buffer object have completed, and may be
     * used to modify and/or query the corresponding range of the data store
     * according to the following flag bits set in $access:
     * {@see GL46::GL_MAP_READ_BIT} indicates that the returned pointer may
     * be used to read buffer object data. No GL error is generated if the
     * pointer is used to query a mapping which excludes this flag, but the
     * result is undefined and system errors (possibly including program
     * termination) may occur.     {@see GL46::GL_MAP_WRITE_BIT} indicates
     * that the returned pointer may be used to modify buffer object data. No
     * GL error is generated if the pointer is used to modify a mapping which
     * excludes this flag, but the result is undefined and system errors
     * (possibly including program termination) may occur.
     * {@see GL46::GL_MAP_PERSISTENT_BIT} indicates that the mapping is to be
     * made in a persistent fassion and that the client intends to hold and
     * use the returned pointer during subsequent GL operation. It is not an
     * error to call drawing commands (render) while buffers are mapped using
     * this flag. It is an error to specify this flag if the buffer's data
     * store was not allocated through a call to the
     * {@see GL46::glBufferStorage} command in which the
     * {@see GL46::GL_MAP_PERSISTENT_BIT} was also set.
     * {@see GL46::GL_MAP_COHERENT_BIT} indicates that a persistent mapping
     * is also to be coherent. Coherent maps guarantee that the effect of
     * writes to a buffer's data store by either the client or server will
     * eventually become visible to the other without further intervention
     * from the application. In the absence of this bit, persistent mappings
     * are not coherent and modified ranges of the buffer store must be
     * explicitly communicated to the GL, either by unmapping the buffer, or
     * through a call to {@see GL46::glFlushMappedBufferRange} or
     * {@see GL46::glMemoryBarrier}.
     *
     * The following optional flag bits in $access may be used to modify the
     * mapping:    {@see GL46::GL_MAP_INVALIDATE_RANGE_BIT} indicates that
     * the previous contents of the specified range may be discarded. Data
     * within this range are undefined with the exception of subsequently
     * written data. No GL error is generated if subsequent GL operations
     * access unwritten data, but the result is undefined and system errors
     * (possibly including program termination) may occur. This flag may not
     * be used in combination with {@see GL46::GL_MAP_READ_BIT}.
     * {@see GL46::GL_MAP_INVALIDATE_BUFFER_BIT} indicates that the previous
     * contents of the entire buffer may be discarded. Data within the entire
     * buffer are undefined with the exception of subsequently written data.
     ...
     * combination with {@see GL46::GL_MAP_READ_BIT}.
     * {@see GL46::GL_MAP_FLUSH_EXPLICIT_BIT} indicates that one or more
     * discrete subranges of the mapping may be modified. When this flag is
     * set, modifications to each subrange must be explicitly flushed by
     * calling {@see GL46::glFlushMappedBufferRange}. No GL error is set if a
     * subrange of the mapping is modified and not flushed, but data within
     * the corresponding subrange of the buffer are undefined. This flag may
     * only be used in conjunction with {@see GL46::GL_MAP_WRITE_BIT}. When
     * this option is selected, flushing is strictly limited to regions that
     * are explicitly indicated with calls to
     * {@see GL46::glFlushMappedBufferRange} prior to unmap; if this option
     * is not selected {@see GL46::glUnmapBuffer} will automatically flush
     * the entire mapped range when called.
     * {@see GL46::GL_MAP_UNSYNCHRONIZED_BIT} indicates that the GL should
     * not attempt to synchronize pending operations on the buffer prior to
     * returning from {@see GL46::glMapBufferRange} or
     * {@see GL46::glMapNamedBufferRange}. No GL error is generated if
     * pending operations which source or modify the buffer overlap the
     * mapped region, but the result of such previous and any subsequent
     ...
     * If no error occurs, the returned pointer will reflect an allocation
     * aligned to the value of {@see GL46::GL_MIN_MAP_BUFFER_ALIGNMENT} basic
     * machine units. Subtracting $offset from this returned pointer will
     * always produce a multiple of the value of
     * {@see GL46::GL_MIN_MAP_BUFFER_ALIGNMENT}.
     *
     * The returned pointer values may not be passed as parameter values to
     * GL commands. For example, they may not be used to specify array
     * pointers, or to specify or query pixel or texture image data; such
    ...
    /**
     * {@see GL46::glUnmapBuffer} and {@see GL46::glUnmapNamedBuffer} unmap
     * (release) any mapping of a specified buffer object into the client's
     * address space (see {@see GL46::glMapBufferRange} and
     ...
     * {@see GL46::GL_MAP_PERSISTENT_BIT} (see
     * {@see GL46::glMapBufferRange}). When a data store is unmapped, the
     * mapped pointer becomes invalid.
     *
     * {@see GL46::glUnmapBuffer} returns {@see GL46::GL_TRUE} unless the
     * data store contents have become corrupt during the time the data store
     * was mapped. This can occur for system-specific reasons that affect the
     * availability of graphics memory, such as screen mode changes. In such
     * situations, {@see GL46::GL_FALSE} is returned and the data store
     * contents are undefined. An application must detect this rare condition
    ...
    /**
     * {@see GL46::glFlushMappedBufferRange} indicates that modifications
     * have been made to a range of a mapped buffer object. The buffer object
     * must previously have been mapped with the
     * {@see GL46::GL_MAP_FLUSH_EXPLICIT_BIT} flag.
     *
     * $offset and $length indicate the modified subrange of the mapping, in
     * basic machine units. The specified subrange to flush is relative to
     ...
     * {@see GL46::GL_MAP_PERSISTENT_BIT} and
     * {@see GL46::GL_MAP_FLUSH_EXPLICIT_BIT} set, then these commands may be
     * called to ensure that data written by the client into the flushed
     * region becomes visible to the server. Data written to a coherent store
     * will always become visible to the server after an unspecified period
     * of time.
     *
    ...
    /**
     * {@see GL46::glGetBufferPointerv} and
     * {@see GL46::glGetNamedBufferPointerv} return the buffer pointer
     * $pname, which must be {@see GL46::GL_BUFFER_MAP_POINTER}. The single
     * buffer map pointer is returned in $params. A {@see GL46::NULL} pointer
     * is returned if the buffer object's data store is not currently mapped;
     * or if the requesting context did not map the buffer object's data
     * store, and the implementation is unable to support mappings on
     * multiple clients.
     *
    ...
    /**
     * {@see GL46::glCreateFramebuffers} returns $n previously unused
     * framebuffer names in $framebuffers, each representing a new
     * framebuffer object initialized to the default state.
     *
    ...
    /**
     * {@see GL46::glFramebufferRenderbuffer} and
     * {@see GL46::glNamedFramebufferRenderbuffer} attaches a renderbuffer as
     ...
     *
     * For {@see GL46::glFramebufferRenderbuffer}, the framebuffer object is
     * that bound to $target, which must be {@see GL46::GL_DRAW_FRAMEBUFFER},
     * {@see GL46::GL_READ_FRAMEBUFFER} or {@see GL46::GL_FRAMEBUFFER}.
     * {@see GL46::GL_FRAMEBUFFER} is equivalent to
     * {@see GL46::GL_DRAW_FRAMEBUFFER}.
     *
     * For {@see GL46::glNamedFramebufferRenderbuffer}, $framebuffer is the
     ...
     * object of type $renderbuffertarget. If $renderbuffer is not zero, then
     * the specified renderbuffer will be used as the logical buffer
     ...
     * or {@see GL46::GL_DEPTH_STENCIL_ATTACHMENT}. i in may range from zero
     * to the value of {@see GL46::GL_MAX_COLOR_ATTACHMENTS} minus one.
     * Setting $attachment to the value
     * {@see GL46::GL_DEPTH_STENCIL_ATTACHMENT} is a special case causing
     * both the depth and stencil attachments of the specified framebuffer
     * object to be set to $renderbuffer, which should have the base internal
     * format {@see GL46::GL_DEPTH_STENCIL}.
     *
     * The value of {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE} for
     * the specified attachment point is set to {@see GL46::GL_RENDERBUFFER}
     * and the value of {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME} is
     * set to $renderbuffer. All other state values of specified attachment
     * point are set to their default values. No change is made to the state
     * of the renderbuuffer object and any previous attachment to the
     * $attachment logical buffer of the specified framebuffer object is
     * broken.
     *
    ...
    /**
     * {@see GL46::glGetFramebufferParameteriv} and
     * {@see GL46::glGetNamedFramebufferParameteriv} query parameters of a
     * specified framebuffer object.
     *
     * For {@see GL46::glGetFramebufferParameteriv}, the framebuffer object
     * is that bound to $target, which must be one of
     ...
     * to {@see GL46::GL_DRAW_FRAMEBUFFER}. Default framebuffers may also be
     * queried if bound to $target.
     *
     * For {@see GL46::glGetNamedFramebufferParameteriv}, $framebuffer is the
     ...
     *    of
     *    {@see GL46::GL_FRAMEBUFFER_DEFAULT_WIDTH} for the framebuffer object.
     *
     *  - {@see GL46::GL_FRAMEBUFFER_DEFAULT_HEIGHT}: $params returns the value
     *    of
     *    {@see GL46::GL_FRAMEBUFFER_DEFAULT_HEIGHT} for the framebuffer object.
     *
     *  - {@see GL46::GL_FRAMEBUFFER_DEFAULT_LAYERS}: $params returns the value
     *    of
     *    {@see GL46::GL_FRAMEBUFFER_DEFAULT_LAYERS} for the framebuffer object.
     *
     *  - {@see GL46::GL_FRAMEBUFFER_DEFAULT_SAMPLES}: $params returns the value
     *    of
     *    {@see GL46::GL_FRAMEBUFFER_DEFAULT_SAMPLES} for the framebuffer
     *    object.
     *
     *  - {@see GL46::GL_FRAMEBUFFER_DEFAULT_FIXED_SAMPLE_LOCATIONS}: $params
     *    returns the boolean value of
     *    {@see GL46::GL_FRAMEBUFFER_DEFAULT_FIXED_SAMPLE_LOCATIONS}.
     *
     ...
     *    GLenum value indicating the preferred pixel data
     *    format for the framebuffer object. See {@see GL46::glReadPixels}.
     *
     *  - {@see GL46::GL_IMPLEMENTATION_COLOR_READ_TYPE}: $param returns a
     *    GLenum value indicating the implementation's
     *    preferred pixel data type for the framebuffer object. See
     *    {@see GL46::glReadPixels}.
     *
     *  - {@see GL46::GL_SAMPLES}: $param returns an integer value indicating
     *    the coverage mask size for
     *    the framebuffer object. See {@see GL46::glSampleCoverage}.
     *
     *  - {@see GL46::GL_SAMPLE_BUFFERS}: $param returns an integer value
     *    indicating the number of sample
     *    buffers associated with the framebuffer object. See
     *    {@see GL46::glSampleCoverage}.
     *
     *  - {@see GL46::GL_STEREO}: $param returns a boolean value indicating
     *    whether stereo buffers
    ...
    /**
     * {@see GL46::glFramebufferTextureLayer} and
     * {@see GL46::glNamedFramebufferTextureLayer} attach a single layer of a
     ...
     *
     * For {@see GL46::glFramebufferTextureLayer}, the framebuffer object is
     * that bound to $target, which must be {@see GL46::GL_DRAW_FRAMEBUFFER},
     * {@see GL46::GL_READ_FRAMEBUFFER}, or {@see GL46::GL_FRAMEBUFFER}.
     * {@see GL46::GL_FRAMEBUFFER} is equivalent to
     * {@see GL46::GL_DRAW_FRAMEBUFFER}.
     *
     * For {@see GL46::glNamedFramebufferTextureLayer}, $framebuffer is the
     * name of the framebuffer object.
     *
     * $attachment specifies the logical attachment of the framebuffer and
     * must be {@see GL46::GL_COLOR_ATTACHMENT}i,
     ...
     * greater than or equal to zero and less than or equal to $log_2$ of the
     * value of {@see GL46::GL_MAX_TEXTURE_SIZE}.
     *
     ...
     * layer = \\left\\lfloor { layer \\over 6 } \\right\\rfloor$$ and $$
     * face = k \\bmod 6. $$
     *
    ...
    /**
     * {@see GL46::glInvalidateSubFramebuffer} and
     * {@see GL46::glInvalidateNamedFramebufferSubData} invalidate the
     * contents of a specified region of a specified set of attachments of a
     * framebuffer.
     *
     * For {@see GL46::glInvalidateSubFramebuffer}, the framebuffer object is
     * that bound to $target, which must be one of
     * {@see GL46::GL_FRAMEBUFFER}, {@see GL46::GL_READ_FRAMEBUFFER} or
     * {@see GL46::GL_DRAW_FRAMEBUFFER}. {@see GL46::GL_FRAMEBUFFER} is
     * equivalent to {@see GL46::GL_DRAW_FRAMEBUFFER}. Default framebuffers
     * may also be invalidated if bound to $target.
     *
     * For {@see GL46::glInvalidateNamedFramebufferSubData}, $framebuffer is
     ...
     * $attachments must be one of {@see GL46::GL_DEPTH_ATTACHMENT},
     * {@see GL46::GL_STENCIL_ATTACHMENT}
     * {@see GL46::GL_DEPTH_STENCIL_ATTACHMENT}, or
     * {@see GL46::GL_COLOR_ATTACHMENT}i, where i is between zero and the
     * value of {@see GL46::GL_MAX_FRAMEBUFFER_ATTACHMENTS} minus one.
     *
     * If the specified framebuffer is a default framebuffer, each element of
     * $attachments must be one of {@see GL46::GL_FRONT_LEFT},
     * {@see GL46::GL_FRONT_RIGHT}, {@see GL46::GL_BACK_LEFT},
     * {@see GL46::GL_BACK_RIGHT}, {@see GL46::GL_AUX}i,
     * {@see GL46::GL_ACCUM}, {@see GL46::GL_COLOR}, {@see GL46::GL_DEPTH},
     * or {@see GL46::GL_STENCIL}. {@see GL46::GL_COLOR}, is treated as
     * {@see GL46::GL_BACK_LEFT} for a double-buffered context and
     * {@see GL46::GL_FRONT_LEFT} for a single-buffered context. The other
     * attachments identify the corresponding specific buffer.
     *
     * The contents of the specified region of each specified attachment
     * become undefined after execution of
     * {@see GL46::glInvalidateSubFramebuffer} or
     * {@see GL46::glInvalidateNamedFramebufferSubData}. The region to be
     * invalidated is specified by $x, $y, $width and $height where $x and $y
     * give the offset from the origin (with lower-left corner at $(0,0)$)
     * and $width and $height are the width and height, respectively, of the
     * region. Any pixels lying outside of the window allocated to the
     * current GL context (for the default framebuffer), or outside of the
     * attachments of the framebuffer object, are ignored. If the framebuffer
     * object is not complete, these commands may be ignored.
     *
     * If the framebuffer object is not complete,
     * {@see GL46::glInvalidateSubFramebuffer} and
     * {@see GL46::glInvalidateNamedFramebufferSubData} may be ignored. This
     * is not an error.
     *
    ...
    /**
     * {@see GL46::glInvalidateBufferSubData} invalidates all or part of the
    ...
    /**
     * {@see GL46::glBlitFramebuffer} and {@see GL46::glBlitNamedFramebuffer}
     * transfer a rectangle of pixel values from one region of a read
     * framebuffer to another region of a draw framebuffer.
     *
     * For {@see GL46::glBlitFramebuffer}, the read and draw framebuffers are
     * those bound to the {@see GL46::GL_READ_FRAMEBUFFER} and
     * {@see GL46::GL_DRAW_FRAMEBUFFER} targets respectively.
     *
     * For {@see GL46::glBlitNamedFramebuffer}, $readFramebuffer and
     * $drawFramebuffer are the names of the read and draw framebuffer
     * objects respectively. If $readFramebuffer or $drawFramebuffer is zero,
     * then the default read or draw framebuffer respectively is used.
     *
     * $mask is the bitwise OR of a number of values indicating which buffers
     * are to be copied. The values are {@see GL46::GL_COLOR_BUFFER_BIT},
     * {@see GL46::GL_DEPTH_BUFFER_BIT}, and
     * {@see GL46::GL_STENCIL_BUFFER_BIT}. The pixels corresponding to these
     * buffers are copied from the source rectangle bounded by the locations
     * ($srcX0, $srcY0) and ($srcX1, $srcY1) to the destination rectangle
     * bounded by the locations ($dstX0, $dstY0) and ($dstX1, $dstY1). The
     * lower bounds of the rectangle are inclusive, while the upper bounds
     * are exclusive.
     *
     ...
     * buffer, and/or the stencil buffer depending on mask. The actual region
     * written to the draw framebuffer is limited to the intersection of the
     * destination buffers being written, which may include multiple draw
     * buffers, the depth buffer, and/or the stencil buffer depending on
     * mask. Whether or not the source or destination regions are altered due
     * to these limits, the scaling and offset applied to pixels being
     ...
     * resize the source image , and must be {@see GL46::GL_NEAREST} or
     * {@see GL46::GL_LINEAR}. {@see GL46::GL_LINEAR} is only a valid
     * interpolation method for the color buffer. If $filter is not
     * {@see GL46::GL_NEAREST} and $mask includes
     * {@see GL46::GL_DEPTH_BUFFER_BIT} or
     * {@see GL46::GL_STENCIL_BUFFER_BIT}, no data is transferred and a
     * {@see GL46::GL_INVALID_OPERATION} error is generated.
     *
     * If $filter is {@see GL46::GL_LINEAR} and the source rectangle would
     * require sampling outside the bounds of the source framebuffer, values
     * are read as if the {@see GL46::GL_CLAMP_TO_EDGE} texture wrapping mode
     * were applied.
     *
     * When the color buffer is transferred, values are taken from the read
     * buffer of the specified read framebuffer and written to each of the
     * draw buffers of the specified draw framebuffer.
     *
    ...
    /**
     * {@see GL46::glCheckFramebufferStatus} and
     * {@see GL46::glCheckNamedFramebufferStatus} return the completeness
     * status of a framebuffer object when treated as a read or draw
     * framebuffer, depending on the value of $target.
     *
     * For {@see GL46::glCheckFramebufferStatus}, the framebuffer checked is
     * that bound to $target, which must be {@see GL46::GL_DRAW_FRAMEBUFFER},
     * {@see GL46::GL_READ_FRAMEBUFFER} or {@see GL46::GL_FRAMEBUFFER}.
     * {@see GL46::GL_FRAMEBUFFER} is equivalent to
     * {@see GL46::GL_DRAW_FRAMEBUFFER}.
     *
     * For {@see GL46::glCheckNamedFramebufferStatus}, $framebuffer is zero
     * or the name of the framebuffer object to check. If $framebuffer is
     * zero, then the status of the default read or draw framebuffer, as
     * determined by $target, is returned.
     *
     * The return value is {@see GL46::GL_FRAMEBUFFER_COMPLETE} if the
     * specified framebuffer is complete. Otherwise, the return value is
     * determined as follows:    {@see GL46::GL_FRAMEBUFFER_UNDEFINED} is
     * returned if the specified framebuffer is the default read or draw
     * framebuffer, but the default framebuffer does not exist.
     * {@see GL46::GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT} is returned if any
     * of the framebuffer attachment points are framebuffer incomplete.
     * {@see GL46::GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT} is returned
     * if the framebuffer does not have at least one image attached to it.
     *  {@see GL46::GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER} is returned if the
     * value of {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE} is
     * {@see GL46::GL_NONE} for any color attachment point(s) named by
     * {@see GL46::GL_DRAW_BUFFERi}.
     * {@see GL46::GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER} is returned if
     * {@see GL46::GL_READ_BUFFER} is not {@see GL46::GL_NONE} and the value
     * of {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE} is
     * {@see GL46::GL_NONE} for the color attachment point named by
     * {@see GL46::GL_READ_BUFFER}.
     * {@see GL46::GL_FRAMEBUFFER_UNSUPPORTED} is returned if the combination
     * of internal formats of the attached images violates an
     * implementation-dependent set of restrictions.
     * {@see GL46::GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE} is returned if the
     * value of {@see GL46::GL_RENDERBUFFER_SAMPLES} is not the same for all
     * attached renderbuffers; if the value of
     * {@see GL46::GL_TEXTURE_SAMPLES} is the not same for all attached
     * textures; or, if the attached images are a mix of renderbuffers and
     * textures, the value of {@see GL46::GL_RENDERBUFFER_SAMPLES} does not
     * match the value of {@see GL46::GL_TEXTURE_SAMPLES}.
     * {@see GL46::GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE} is also returned if
     * the value of {@see GL46::GL_TEXTURE_FIXED_SAMPLE_LOCATIONS} is not the
     * same for all attached textures; or, if the attached images are a mix
     * of renderbuffers and textures, the value of
     * {@see GL46::GL_TEXTURE_FIXED_SAMPLE_LOCATIONS} is not
     * {@see GL46::GL_TRUE} for all attached textures.
     * {@see GL46::GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS} is returned if
     * any framebuffer attachment is layered, and any populated attachment is
    ...
    /**
     * {@see GL46::glGetFramebufferAttachmentParameteriv} and
     * {@see GL46::glGetNamedFramebufferAttachmentParameteriv} return
     * parameters of attachments of a specified framebuffer object.
     *
     * For {@see GL46::glGetFramebufferAttachmentParameteriv}, the
     * framebuffer object is that bound to $target, which must be one of
     * {@see GL46::GL_DRAW_FRAMEBUFFER}, {@see GL46::GL_READ_FRAMEBUFFER} or
     * {@see GL46::GL_FRAMEBUFFER}. {@see GL46::GL_FRAMEBUFFER} is equivalent
     * to {@see GL46::GL_DRAW_FRAMEBUFFER}. Buffers of default framebuffers
     * may also be queried if bound to $target.
     *
     * For {@see GL46::glGetNamedFramebufferAttachmentParameteriv},
     ...
     * be one of {@see GL46::GL_DEPTH_ATTACHMENT},
     * {@see GL46::GL_STENCIL_ATTACHMENT}
     * {@see GL46::GL_DEPTH_STENCIL_ATTACHMENT}, or
     * {@see GL46::GL_COLOR_ATTACHMENT}i, where i is between zero and the
     * value of {@see GL46::GL_MAX_COLOR_ATTACHMENTS} minus one.
     *
     * If the specified framebuffer is a default framebuffer, $target,
     * $attachment must be one of {@see GL46::GL_FRONT_LEFT},
     * {@see GL46::GL_FRONT_RIGHT}, {@see GL46::GL_BACK_LEFT},
     * {@see GL46::GL_BACK_RIGHT}, {@see GL46::GL_DEPTH} or
     * {@see GL46::GL_STENCIL}, identifying the corresponding buffer.
     *
     * If $attachment is {@see GL46::GL_DEPTH_STENCIL_ATTACHMENT}, the same
     * object must be bound to both the depth and stencil attachment points
     * of the framebuffer object, and information about that object is
     * returned.
     *
     * Upon successful return, if $pname is
     * {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE}, then $params will
     * contain one of {@see GL46::GL_NONE},
     * {@see GL46::GL_FRAMEBUFFER_DEFAULT}, {@see GL46::GL_TEXTURE}, or
     * {@see GL46::GL_RENDERBUFFER}, identifying the type of object which
     * contains the attached image. Other values accepted for $pname depend
     * on the type of object, as described below.
     *
     * If the value of {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE} is
     * {@see GL46::GL_NONE}, then either no framebuffer is bound to $target;
     * or a default framebuffer is queried, $attachment is
     * {@see GL46::GL_DEPTH} or {@see GL46::GL_STENCIL}, and the number of
     * depth or stencil bits, respectively, is zero. In this case querying
     * $pname {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME} will return
     * zero, and all other queries will generate an error.
     *
     * If the value of {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE} is
     * not {@see GL46::GL_NONE}, these queries apply to all other framebuffer
     * types:
     *
     *  -  If $pname is {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_RED_SIZE},
     *    {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_GREEN_SIZE},
     *    {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_BLUE_SIZE},
     *    {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_ALPHA_SIZE},
     *    {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_DEPTH_SIZE} or
     *    {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_STENCIL_SIZE}, then $params will
     *    contain the number of bits in the corresponding red, green, blue,
     *    alpha, depth, or stencil component of the specified attachment. If the
     *    requested component is not present in the attachment, or if no data
     *    storage or texture image has been specified for the attachment, then
     *    $params will contain zero.
     *  -  If $pname is {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_COMPONENT_TYPE},
     *    then $params will contain the format of components of the specified
     *    attachment, one of {@see GL46::GL_FLOAT}, {@see GL46::GL_INT},
     *    {@see GL46::GL_UNSIGNED_INT}, {@see GL46::GL_SIGNED_NORMALIZED}, or
     *    {@see GL46::GL_UNSIGNED_NORMALIZED} for floating-point, signed
     *    integer, unsigned integer, signed normalized fixed-point, or unsigned
     *    normalized fixed-point components respectively. Only color buffers may
     *    have integer components. If no data storage or texture image has been
     *    specified for the attachment, then $params will contain
     *    {@see GL46::GL_NONE}. This query cannot be performed for a combined
     ...
     *    attachment, one of {@see GL46::GL_LINEAR} or {@see GL46::GL_SRGB} for
     *    linear or sRGB-encoded components, respectively. Only color buffer
     *    components may be sRGB-encoded; such components are treated as
     *    described in the OpenGL Specification. For a default framebuffer,
     ...
     *    attachment is not a color attachment, or if no data storage or texture
     *    image has been specified for the attachment, then $params will contain
     *    {@see GL46::GL_LINEAR}.
     *
     * If the value of {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE} is
     * {@see GL46::GL_RENDERBUFFER}, then
     *
     *  -  If $pname is {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME},
     *    $params will contain the name of the renderbuffer object which
     *    contains the attached image.
     *
     * If the value of {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE} is
     * {@see GL46::GL_TEXTURE}, then
     *
     ...
     *    contains the attached image.
     *  -  If $pname is
     *    {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_CUBE_MAP_FACE} and the
     *    value of {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME} is the
     *    name of a cube map texture object, then $params will contain the cube
     *    map face of the cubemap texture object which contains the attached
     *    image. Otherwise $params will contain zero.
     *  -  If $pname is {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_LAYERED}, then
     *    $params will contain {@see GL46::GL_TRUE} if an entire level of a
     *    three-dimensional texture, cube map texture, or one-or two-dimensional
     *    array texture is attached. Otherwise, $params will contain
     *    {@see GL46::GL_FALSE}.
     *  -  If $pname is {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_LAYER};
     *    the value of {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME} is the
     *    name of a three-dimensional, or a one- or two-dimensional array
     *    texture; and the value of
     *    {@see GL46::GL_FRAMEBUFFER_ATTACHMENT_LAYERED} is
    ...
    /**
     * {@see GL46::glCreateRenderbuffers} returns $n previously unused
    ...
    /**
     * {@see GL46::glRenderbufferStorage} is equivalent to calling
     * {@see GL46::glRenderbufferStorageMultisample} with the $samples set to
     * zero, and {@see GL46::glNamedRenderbufferStorage} is equivalent to
     * calling {@see GL46::glNamedRenderbufferStorageMultisample} with the
     * samples set to zero.
     *
     * For {@see GL46::glRenderbufferStorage}, the target of the operation,
     * specified by $target must be {@see GL46::GL_RENDERBUFFER}. For
     * {@see GL46::glNamedRenderbufferStorage}, $renderbuffer must be a name
     * of an existing renderbuffer object. $internalformat specifies the
     * internal format to be used for the renderbuffer object's storage and
     * must be a color-renderable, depth-renderable, or stencil-renderable
     * format. $width and $height are the dimensions, in pixels, of the
     * renderbuffer. Both $width and $height must be less than or equal to
     * the value of {@see GL46::GL_MAX_RENDERBUFFER_SIZE}.
     *
     * Upon success, {@see GL46::glRenderbufferStorage} and
     * {@see GL46::glNamedRenderbufferStorage} delete any existing data store
     * for the renderbuffer image and the contents of the data store after
     * calling {@see GL46::glRenderbufferStorage} are undefined.
     *
    ...
    /**
     * {@see GL46::glRenderbufferStorageMultisample} and
     * {@see GL46::glNamedRenderbufferStorageMultisample} establish the data
     * storage, format, dimensions and number of samples of a renderbuffer
     * object's image.
     *
     * For {@see GL46::glRenderbufferStorageMultisample}, the target of the
     * operation, specified by $target must be {@see GL46::GL_RENDERBUFFER}.
     * For {@see GL46::glNamedRenderbufferStorageMultisample}, $renderbuffer
     * must be an ID of an existing renderbuffer object. $internalformat
     * specifies the internal format to be used for the renderbuffer object's
     * storage and must be a color-renderable, depth-renderable, or
     * stencil-renderable format. $width and $height are the dimensions, in
     * pixels, of the renderbuffer. Both $width and $height must be less than
     * or equal to the value of {@see GL46::GL_MAX_RENDERBUFFER_SIZE}.
     * $samples specifies the number of samples to be used for the
     * renderbuffer object's image, and must be less than or equal to the
     * value of {@see GL46::GL_MAX_SAMPLES}. If $internalformat is a signed
     * or unsigned integer format then $samples must be less than or equal to
     * the value of {@see GL46::GL_MAX_INTEGER_SAMPLES}.
     *
     * Upon success, {@see GL46::glRenderbufferStorageMultisample} and
     * {@see GL46::glNamedRenderbufferStorageMultisample} delete any existing
    ...
    /**
     * {@see GL46::glGetRenderbufferParameteriv} and
     * {@see GL46::glGetNamedRenderbufferParameteriv} query parameters of a
     * specified renderbuffer object.
     *
     * For {@see GL46::glGetRenderbufferParameteriv}, the renderbuffer object
     * is that bound to $target, which must be {@see GL46::GL_RENDERBUFFER}.
     *
     * For {@see GL46::glGetNamedRenderbufferParameteriv}, $renderbuffer is
     ...
     *    returns the width in pixels, the height in pixels, internal
     *    format, or the number of samples, respectively, of the image of the
     *    specified renderbuffer object.
     *
     ...
     *    returns the actual resolution in bits (not the resolution
     *    specified when the image was defined) for the red, green, blue, alpha,
    ...
    /**
     * {@see GL46::glCreateTextures} returns $n previously unused texture
     * names in $textures, each representing a new texture object of the
     * dimensionality and type specified by $target and initialized to the
     * default values for that texture type.
     *
     * $target must be one of {@see GL46::GL_TEXTURE_1D},
     * {@see GL46::GL_TEXTURE_2D}, {@see GL46::GL_TEXTURE_3D},
     * {@see GL46::GL_TEXTURE_1D_ARRAY}, {@see GL46::GL_TEXTURE_2D_ARRAY},
     * {@see GL46::GL_TEXTURE_RECTANGLE}, {@see GL46::GL_TEXTURE_CUBE_MAP},
     * {@see GL46::GL_TEXTURE_CUBE_MAP_ARRAY},
     * {@see GL46::GL_TEXTURE_BUFFER}, {@see GL46::GL_TEXTURE_2D_MULTISAMPLE}
     * or {@see GL46::GL_TEXTURE_2D_MULTISAMPLE_ARRAY}.
     *
     ...
     * fetch in a subsequent drawing command, call
     * {@see GL46::glTextureBarrier} between the two drawing commands to
     * guarantee that writes have completed and caches have been invalidated
    ...
    /**
     * {@see GL46::glTexBufferRange} and {@see GL46::glTextureBufferRange}
     * attach a range of the data store of a specified buffer object to a
     * specified texture object, and specify the storage format for the
     * texture image found in the buffer object. The texture object must be a
     * buffer texture.
     *
     ...
     * {@see GL46::GL_BUFFER_SIZE} for $buffer. Furthermore, $offset must be
     * an integer multiple of the value of
     * {@see GL46::GL_TEXTURE_BUFFER_OFFSET_ALIGNMENT}.
     *
     ...
     * \\times sizeof(base\\_type) } } \\right\\rfloor $$ where $components$
     * and $base\\_type$ are the element count and base data type for
     * elements, as specified in the table above. The number of texels in the
     * texel array is then clamped to the value of the
    ...
    /**
     * {@see GL46::glTexStorage1D} and {@see GL46::glTextureStorage1D}
     ...
     * the format and dimensions of all levels become immutable unless it is
     * a proxy texture. The contents of the image may still be modified,
     * however, its storage requirements may not change. Such a texture is
     * referred to as an immutable-format texture.
     *
     * Calling {@see GL46::glTexStorage1D} is equivalent, assuming no errors
     * are generated, to executing the following pseudo-code:
     *
     * <code>
     *     for (i = 0; i < levels; i++) {
     ...
     *     }
     * </code>
     * Calling {@see GL46::glTextureStorage1D} is equivalent to the above
     * pseudo-code, where $target is the effective target of $texture and it
     * is as if $texture were bound to $target for the purposes of
     * {@see GL46::glTexImage1D}.
     *
     * Since no texture data is actually provided, the values used in the
     * pseudo-code for $format and $type are irrelevant and may be considered
     * to be any values that are legal for the chosen $internalformat
     * enumerant. $internalformat must be one of the sized internal formats
     * given in Table 1 below, one of the sized depth-component formats
     * {@see GL46::GL_DEPTH_COMPONENT32F}, {@see GL46::GL_DEPTH_COMPONENT24},
     * or {@see GL46::GL_DEPTH_COMPONENT16}, one of the combined
     * depth-stencil formats, {@see GL46::GL_DEPTH32F_STENCIL8}, or
     * {@see GL46::GL_DEPTH24_STENCIL8}, or the stencil-only format,
     * {@see GL46::GL_STENCIL_INDEX8}. Upon success, the value of
     * {@see GL46::GL_TEXTURE_IMMUTABLE_FORMAT} becomes {@see GL46::GL_TRUE}.
     * The value of {@see GL46::GL_TEXTURE_IMMUTABLE_FORMAT} may be
     * discovered by calling {@see GL46::glGetTexParameter} with $pname set
     * to {@see GL46::GL_TEXTURE_IMMUTABLE_FORMAT}. No further changes to the
     * dimensions or format of the texture object may be made. Using any
     * command that might alter the dimensions or format of the texture
     * object (such as {@see GL46::glTexImage1D} or another call to
     * {@see GL46::glTexStorage1D}) will result in the generation of a
     * {@see GL46::GL_INVALID_OPERATION} error, even if it would not, in
     * fact, alter the dimensions or format of the object.
     *
    ...
    /**
     * {@see GL46::glTexStorage2D} and {@see GL46::glTextureStorage2D}
     * specify the storage requirements for all levels of a two-dimensional
     * texture or one-dimensional texture array simultaneously. Once a
     * texture is specified with this command, the format and dimensions of
     * all levels become immutable unless it is a proxy texture. The contents
     * of the image may still be modified, however, its storage requirements
     * may not change. Such a texture is referred to as an immutable-format
     * texture.
     *
     * The behavior of {@see GL46::glTexStorage2D} depends on the $target
     * parameter. When $target is {@see GL46::GL_TEXTURE_2D},
     * {@see GL46::GL_PROXY_TEXTURE_2D}, {@see GL46::GL_TEXTURE_RECTANGLE},
     * {@see GL46::GL_PROXY_TEXTURE_RECTANGLE} or
     * {@see GL46::GL_PROXY_TEXTURE_CUBE_MAP}, calling
     * {@see GL46::glTexStorage2D} is equivalent, assuming no errors are
     ...
     *     }
     * </code>
     * When $target is {@see GL46::GL_TEXTURE_CUBE_MAP},
     ...
     *     }
     * </code>
     * When $target is {@see GL46::GL_TEXTURE_1D_ARRAY} or
     * {@see GL46::GL_PROXY_TEXTURE_1D_ARRAY}, {@see GL46::glTexStorage2D} is
     * equivalent to:
     *
     * <code>
     *     for (i = 0; i < levels; i++) {
     ...
     *     }
     * </code>
     * Calling {@see GL46::glTextureStorage2D} is equivalent to the above
     * pseudo-code, where $target is the effective target of $texture and it
     * is as if $texture were bound to $target for the purposes of
     ...
     * dimensions or format of the texture object may be made. Using any
     * command that might alter the dimensions or format of the texture
     * object (such as {@see GL46::glTexImage2D} or another call to
     * {@see GL46::glTexStorage2D}) will result in the generation of a
     * {@see GL46::GL_INVALID_OPERATION} error, even if it would not, in
     * fact, alter the dimensions or format of the object.
     *
    ...
    /**
     * {@see GL46::glTexStorage3D} and {@see GL46::glTextureStorage3D}
     * specify the storage requirements for all levels of a
     ...
     * referred to as an immutable-format texture.
     *
     * The behavior of {@see GL46::glTexStorage3D} depends on the $target
     * parameter. When $target is {@see GL46::GL_TEXTURE_3D}, or
     * {@see GL46::GL_PROXY_TEXTURE_3D}, calling {@see GL46::glTexStorage3D}
     * is equivalent, assuming no errors are generated, to executing the
     ...
     *     }
     * </code>
     * When $target is {@see GL46::GL_TEXTURE_2D_ARRAY},
     * {@see GL46::GL_PROXY_TEXTURE_2D_ARRAY},
     * {@see GL46::GL_TEXTURE_CUBE_MAP_ARRAY}, or
     * {@see GL46::GL_PROXY_TEXTURE_CUBE_MAP_ARRAY},
     ...
     *     }
     * </code>
     * Calling {@see GL46::glTextureStorage3D} is equivalent to the above
     * pseudo-code, where $target is the effective target of $texture and it
     * is as if $texture were bound to $target for the purposes of
     ...
     * dimensions or format of the texture object may be made. Using any
     * command that might alter the dimensions or format of the texture
     * object (such as {@see GL46::glTexImage3D} or another call to
     * {@see GL46::glTexStorage3D}) will result in the generation of a
     * {@see GL46::GL_INVALID_OPERATION} error, even if it would not, in
     * fact, alter the dimensions or format of the object.
     *
    ...
    /**
     * {@see GL46::glTexStorage2DMultisample} and
     * {@see GL46::glTextureStorage2DMultisample} specify the storage
     * requirements for a two-dimensional multisample texture. Once a texture
     * is specified with this command, its format and dimensions become
     ...
     *
     * $samples specifies the number of samples to be used for the texture
     * and must be greater than zero and less than or equal to the value of
     * {@see GL46::GL_MAX_SAMPLES}. $internalformat must be a
     ...
     * texture. If $fixedsamplelocations is {@see GL46::GL_TRUE}, the image
     * will use identical sample locations and the same number of samples for
     * all texels in the image, and the sample locations will not depend on
    ...
    /**
     * {@see GL46::glTexStorage3DMultisample} and
     * {@see GL46::glTextureStorage3DMultisample} specify the storage
     * requirements for a two-dimensional multisample array texture. Once a
     * texture is specified with this command, its format and dimensions
     * become immutable unless it is a proxy texture. The contents of the
     ...
     * color-renderable, depth-renderable, or stencil-renderable format.
     * $width and $height specify the width and height, respectively, of the
     * texture and $depth specifies the depth (or the number of layers) of
     * the texture. If $fixedsamplelocations is {@see GL46::GL_TRUE}, the
     * image will use identical sample locations and the same number of
     * samples for all texels in the image, and the sample locations will not
     ...
     * disable one-dimensional texturing, call {@see GL46::glEnable} and
     * {@see GL46::glDisable} with argument {@see GL46::GL_TEXTURE_1D}.
     *
     * {@see GL46::glTexSubImage1D} and {@see GL46::glTextureSubImage1D}
     ...
     * existing texture array with x indices $xoffset and    xoffset + width
     * - 1  , inclusive. This region may not include any texels outside the
     * range of the texture array as it was originally specified. It is not
     * an error to specify a subtexture with width of 0, but such a
     * specification has no effect.
     *
     * If a non-zero named buffer object is bound to the
     * {@see GL46::GL_PIXEL_UNPACK_BUFFER} target (see
     * {@see GL46::glBindBuffer}) while a texture image is specified, $pixels
     ...
     *
     * {@see GL46::glTexSubImage2D} and {@see GL46::glTextureSubImage2D}
     ...
     * $xoffset and    xoffset + width - 1  , inclusive, and y indices
     * $yoffset and    yoffset + height - 1  , inclusive. This region may not
     ...
     * graphical primitive for which texturing is enabled.
     *
     * {@see GL46::glTexSubImage3D} and {@see GL46::glTextureSubImage3D}
     ...
     * replace the portion of the existing texture array with x indices
     * $xoffset and    xoffset + width - 1  , inclusive, y indices $yoffset
     * and    yoffset + height - 1  , inclusive, and z indices $zoffset and
     *  zoffset + depth - 1  , inclusive. For three-dimensional textures, the
     * z index refers to the third dimension. For two-dimensional array
     * textures, the z index refers to the slice index. This region may not
     * include any texels outside the range of the texture array as it was
     * originally specified. It is not an error to specify a subtexture with
    ...
    /**
     * {@see GL46::glCopyTexSubImage1D} and
     * {@see GL46::glCopyTextureSubImage1D} replace a portion of a
     * one-dimensional texture image with pixels from the current
     * {@see GL46::GL_READ_BUFFER} (rather than from main memory, as is the
     * case for {@see GL46::glTexSubImage1D}). For
     * {@see GL46::glCopyTexSubImage1D}, the texture object that is bound to
     * $target will be used for the process. For
     * {@see GL46::glCopyTextureSubImage1D}, $texture tells which texture
     * object should be used for the purpose of the call.
     *
     ...
     * in the texture array may not include any texels outside the texture
     ...
     * row of the current {@see GL46::GL_READ_BUFFER} are outside the read
    ...
    /**
     * {@see GL46::glCopyTexSubImage2D} and
     * {@see GL46::glCopyTextureSubImage2D} replace a rectangular portion of
     * a two-dimensional texture image, cube-map texture image, rectangular
     * image, or a linear portion of a number of slices of a one-dimensional
     * array texture with pixels from the current {@see GL46::GL_READ_BUFFER}
     * (rather than from main memory, as is the case for
     * {@see GL46::glTexSubImage2D}).
     *
     ...
     *
     * When $target is {@see GL46::GL_TEXTURE_1D_ARRAY} then the y coordinate
     * and height are treated as the start slice and number of slices to
     * modify.
     *
     * If any of the pixels within the specified rectangle of the current
     * {@see GL46::GL_READ_BUFFER} are outside the read window associated
    ...
    /**
     * {@see GL46::glCopyTexSubImage3D} and
     * {@see GL46::glCopyTextureSubImage3D} functions replace a rectangular
     * portion of a three-dimensional or two-dimensional array texture image
     * with pixels from the current {@see GL46::GL_READ_BUFFER} (rather than
     * from main memory, as is the case for {@see GL46::glTexSubImage3D}).
     *
     * The screen-aligned pixel rectangle with lower left corner at ($x, $y)
     * and with width $width and height $height replaces the portion of the
     * texture array with x indices $xoffset through    xoffset + width - 1
    ...
    /**
     * {@see GL46::glTexParameter} and {@see GL46::glTextureParameter} assign
     * the value or values in $params to the texture parameter specified as
     * $pname. For {@see GL46::glTexParameter}, $target defines the target
     * texture, either {@see GL46::GL_TEXTURE_1D},
     * {@see GL46::GL_TEXTURE_1D_ARRAY}, {@see GL46::GL_TEXTURE_2D},
     * {@see GL46::GL_TEXTURE_2D_ARRAY},
     * {@see GL46::GL_TEXTURE_2D_MULTISAMPLE},
     * {@see GL46::GL_TEXTURE_2D_MULTISAMPLE_ARRAY},
     * {@see GL46::GL_TEXTURE_3D}, {@see GL46::GL_TEXTURE_CUBE_MAP},
     * {@see GL46::GL_TEXTURE_CUBE_MAP_ARRAY}, or
     * {@see GL46::GL_TEXTURE_RECTANGLE}. The following symbols are accepted
     * in $pname:
     *
     *  - {@see GL46::GL_DEPTH_STENCIL_TEXTURE_MODE}: Specifies the mode used to
     *    read from depth-stencil format textures.
     *    $params must be one of {@see GL46::GL_DEPTH_COMPONENT} or
     *    {@see GL46::GL_STENCIL_INDEX}. If the depth stencil mode is
     *    {@see GL46::GL_DEPTH_COMPONENT}, then reads from depth-stencil format
     *    textures will return the depth component of the texel in Rt and the
     ...
     *    to match the texture's internal format and substituted for the
     *    non-existent texel data. If the texture contains depth components, the
     *    first component of {@see GL46::GL_TEXTURE_BORDER_COLOR} is interpreted
     *    as a depth value. The initial value is     0.0 , 0.0 , 0.0 , 0.0    .
     *     If the values for {@see GL46::GL_TEXTURE_BORDER_COLOR} are specified
     *    with {@see GL46::glTexParameterIiv} or
     *    {@see GL46::glTexParameterIuiv}, the values are stored unmodified with
     *    an internal data type of integer. If specified with
     *    {@see GL46::glTexParameteriv}, they are converted to floating point
     *    with the following equation:   f =   2 c + 1    2 b  - 1    . If
     *    specified with {@see GL46::glTexParameterfv}, they are stored
     *    unmodified as floating-point values.
     *
     *  - {@see GL46::GL_TEXTURE_COMPARE_FUNC}: Specifies the comparison
     *    operator used when
     *    {@see GL46::GL_TEXTURE_COMPARE_MODE} is set to
     *    {@see GL46::GL_COMPARE_REF_TO_TEXTURE}. Permissible values are:
     *     Texture Comparison Function     Computed result
     *    {@see GL46::GL_LEQUAL}      result =     1.0   0.0   &amp;it;     r
     *    &lt;=  D t       r &gt;  D t              {@see GL46::GL_GEQUAL}
     *    result =     1.0   0.0   &amp;it;     r &gt;=  D t       r &lt;  D t
     *               {@see GL46::GL_LESS}      result =     1.0   0.0   &amp;it;
     *        r &lt;  D t       r &gt;=  D t
     *    {@see GL46::GL_GREATER}      result =     1.0   0.0   &amp;it;     r
     *    &gt;  D t       r &lt;=  D t              {@see GL46::GL_EQUAL}
     *    result =     1.0   0.0   &amp;it;     r =  D t       r ≠  D t
     *          {@see GL46::GL_NOTEQUAL}      result =     1.0   0.0   &amp;it;
     *       r ≠  D t       r =  D t              {@see GL46::GL_ALWAYS}
     *    result = 1.0        {@see GL46::GL_NEVER}      result = 0.0
     ...
     *    texture. result is assigned to the red channel.
     *
     ...
     *    {@see GL46::GL_DEPTH_COMPONENT_*}; see {@see GL46::glTexImage2D})
     *    Permissible values are:   {@see GL46::GL_COMPARE_REF_TO_TEXTURE}
     ...
     *    See the discussion of {@see GL46::GL_TEXTURE_COMPARE_FUNC} for details
     *    of how the comparison is evaluated. The result of the comparison is
     *    assigned to the red channel.     {@see GL46::GL_NONE}   Specifies that
     *    the red channel should be assigned the appropriate value from the
     *    currently bound depth texture.
     *
     *  - {@see GL46::GL_TEXTURE_LOD_BIAS}: $params specifies a fixed bias value
     *    that is to be added to the
     *    level-of-detail parameter for the texture before texture sampling. The
     *    specified value is added to the shader-supplied bias value (if any)
     *    and subsequently clamped into the implementation-defined range     -
     *    bias max        bias max      , where   bias max    is the value of
     *    the implementation defined constant
     *    {@see GL46::GL_MAX_TEXTURE_LOD_BIAS}. The initial value is 0.0.
     *
     *  - {@see GL46::GL_TEXTURE_MIN_FILTER}: The texture minifying function is
     *    used whenever the level-of-detail
     ...
     *    average of multiple texture elements to compute the texture value. The
     *    other four use mipmaps.   A mipmap is an ordered set of arrays
     *    representing the same image at progressively lower resolutions. If the
     *    texture has dimensions    2 n  × 2 m   , there are     max &amp;af;
     *    n m   + 1   mipmaps. The first mipmap is the original texture, with
     *    dimensions    2 n  × 2 m   . Each subsequent mipmap has dimensions
     *    2   k - 1    × 2   l - 1     , where    2 k  × 2 l    are the
     *    dimensions of the previous mipmap, until either    k = 0   or    l = 0
     *     . At that point, subsequent mipmaps have dimension    1 × 2   l - 1
     *        or    2   k - 1    × 1   until the final mipmap, which has
     *    dimension    1 × 1  . To define the mipmaps, call
     *    {@see GL46::glTexImage1D}, {@see GL46::glTexImage2D},
     *    {@see GL46::glTexImage3D}, {@see GL46::glCopyTexImage1D}, or
     *    {@see GL46::glCopyTexImage2D} with the level argument indicating the
     *    order of the mipmaps. Level 0 is the original texture; level    max
     *    &amp;af;  n m    is the final    1 × 1   mipmap.   $params supplies a
     ...
     *    coordinates.     {@see GL46::GL_LINEAR}   Returns the weighted average
     *    of the four texture elements that are closest to the specified texture
     *    coordinates. These can include items wrapped or repeated from other
     *    parts of a texture, depending on the values of
     *    {@see GL46::GL_TEXTURE_WRAP_S} and {@see GL46::GL_TEXTURE_WRAP_T}, and
     *    on the exact mapping.     {@see GL46::GL_NEAREST_MIPMAP_NEAREST}
     ...
     *    Chooses the mipmap that most closely matches the size of the pixel
     *    being textured and uses the {@see GL46::GL_LINEAR} criterion (a
     ...
     *    {@see GL46::GL_NEAREST_MIPMAP_LINEAR}   Chooses the two mipmaps that
     ...
     *    values.     {@see GL46::GL_LINEAR_MIPMAP_LINEAR}   Chooses the two
     *    mipmaps that most closely match the size of the pixel being textured
     *    and uses the {@see GL46::GL_LINEAR} criterion (a weighted average of
     ...
     *    texture value is a weighted average of those two values.       As more
     *    texture elements are sampled in the minification process, fewer
     *    aliasing artifacts will be apparent. While the {@see GL46::GL_NEAREST}
     *    and {@see GL46::GL_LINEAR} minification functions can be faster than
     *    the other four, they sample only one or multiple texture elements to
     ...
     *    parameter. This floating-point value
     ...
     *    applied to the r component of a texel
     *    before it is returned to the shader. Valid values for $param are
     *    {@see GL46::GL_RED}, {@see GL46::GL_GREEN}, {@see GL46::GL_BLUE},
     *    {@see GL46::GL_ALPHA}, {@see GL46::GL_ZERO} and {@see GL46::GL_ONE}.
     *    If {@see GL46::GL_TEXTURE_SWIZZLE_R} is {@see GL46::GL_RED}, the value
     *    for r will be taken from the first channel of the fetched texel. If
     ...
     *    {@see GL46::GL_TEXTURE_SWIZZLE_R} is {@see GL46::GL_BLUE}, the value
     *    for r will be taken from the third channel of the fetched texel. If
     *    {@see GL46::GL_TEXTURE_SWIZZLE_R} is {@see GL46::GL_ALPHA}, the value
     *    for r will be taken from the fourth channel of the fetched texel. If
     ...
     *    are generated) to setting the parameters of each of
     *    {@see GL46::GL_TEXTURE_SWIZZLE_R}, {@see GL46::GL_TEXTURE_SWIZZLE_G},
     *    {@see GL46::GL_TEXTURE_SWIZZLE_B}, and
     *    {@see GL46::GL_TEXTURE_SWIZZLE_A} successively.
     *
     *  - {@see GL46::GL_TEXTURE_WRAP_S}: Sets the wrap parameter for texture
     *    coordinate s to either
     *    {@see GL46::GL_CLAMP_TO_EDGE}, {@see GL46::GL_CLAMP_TO_BORDER},
     *    {@see GL46::GL_MIRRORED_REPEAT}, {@see GL46::GL_REPEAT}, or
     ...
     *    clamping. {@see GL46::GL_CLAMP_TO_BORDER} evaluates s coordinates in a
     *    similar manner to {@see GL46::GL_CLAMP_TO_EDGE}. However, in cases
     *    where clamping would have occurred in {@see GL46::GL_CLAMP_TO_EDGE}
     *    mode, the fetched texel data is substituted with the values specified
     *    by {@see GL46::GL_TEXTURE_BORDER_COLOR}. {@see GL46::GL_REPEAT} causes
     *    the integer part of the s coordinate to be ignored; the GL uses only
     *    the fractional part, thereby creating a repeating pattern.
     *    {@see GL46::GL_MIRRORED_REPEAT} causes the s coordinate to be set to
     *    the fractional part of the texture coordinate if the integer part of s
     *    is even; if the integer part of s is odd, then the s texture
     *    coordinate is set to    1 -  frac &amp;af;  s    , where    frac
     *    &amp;af;  s    represents the fractional part of s.
     *    {@see GL46::GL_MIRROR_CLAMP_TO_EDGE} causes the s coordinate to be
     *    repeated as for {@see GL46::GL_MIRRORED_REPEAT} for one repetition of
     *    the texture, at which point the coordinate to be clamped as in
     *    {@see GL46::GL_CLAMP_TO_EDGE}. Initially,
     *    {@see GL46::GL_TEXTURE_WRAP_S} is set to {@see GL46::GL_REPEAT}.
     *
     *  - {@see GL46::GL_TEXTURE_WRAP_T}: Sets the wrap parameter for texture
     *    coordinate t to either
     *    {@see GL46::GL_CLAMP_TO_EDGE}, {@see GL46::GL_CLAMP_TO_BORDER},
     *    {@see GL46::GL_MIRRORED_REPEAT}, {@see GL46::GL_REPEAT}, or
     *    {@see GL46::GL_MIRROR_CLAMP_TO_EDGE}. See the discussion under
     *    {@see GL46::GL_TEXTURE_WRAP_S}. Initially,
     *    {@see GL46::GL_TEXTURE_WRAP_T} is set to {@see GL46::GL_REPEAT}.
     *
     *  - {@see GL46::GL_TEXTURE_WRAP_R}: Sets the wrap parameter for texture
     *    coordinate r to either
     *    {@see GL46::GL_CLAMP_TO_EDGE}, {@see GL46::GL_CLAMP_TO_BORDER},
     *    {@see GL46::GL_MIRRORED_REPEAT}, {@see GL46::GL_REPEAT}, or
     *    {@see GL46::GL_MIRROR_CLAMP_TO_EDGE}. See the discussion under
     *    {@see GL46::GL_TEXTURE_WRAP_S}. Initially,
    ...
    /**
     * {@see GL46::glGenerateMipmap} and {@see GL46::glGenerateTextureMipmap}
     * generates mipmaps for the specified texture object. For
     * {@see GL46::glGenerateMipmap}, the texture object that is bound to
     * $target. For {@see GL46::glGenerateTextureMipmap}, $texture is the
     ...
     * computed by repeated, filtered reduction of the $level_{base} + 1$
     * image. For one- and two-dimensional array and cube map array textures,
    ...
    /**
     * {@see GL46::glBindTextureUnit} binds an existing texture object to the
     ...
     * the targets enumerated at the beginning of this section is reset to
     * its default texture for the corresponding texture image unit.
     *
    ...
    /**
     * {@see GL46::glGetTextureSubImage} returns a texture subimage into
     * pixels.
     *
     ...
     * value of {@see GL46::GL_TEXTURE_TARGET} for texture. $Level, $format,
     * $type and $pixels have the same meaning as for
     * {@see GL46::glGetTexImage}. $bufSize is the size of the buffer to
     * receive the retrieved pixel data.
     *
     * For cube map textures, the behavior is as though
     * {@see GL46::GetTextureImage} were called, but only texels from the
     * requested cube map faces (selected by $zoffset and $depth, as
     * described below) were returned.
     *
     ...
     * {@see GL46::glTexSubImage3D}, though for one- and two-dimensional
     * textures there are extra restrictions, described in the errors section
     * below.
     *
     ...
     *
     * For cube map array textures, $zoffset is the first layer-face to
     * access, and $depth is the number of layer-faces to access. A
     * layer-face described by $k$ is translated into an array layer and face
     * according to $$ layer = \\left\\lfloor { layer \\over 6 }
     * \\right\\rfloor$$ and $$ face = k \\bmod 6. $$
     *
     * Component groups from the specified sub-region are packed and placed
     * into memory as described for {@see GL46::glGetTextureImage}, starting
     * with the texel at ($xoffset, $yoffset, $zoffset).
     *
    ...
    /**
     * {@see GL46::glGetCompressedTextureSubImage} can be used to obtain a
     * sub-region of a compressed texture image instead of the whole image,
     * as long as the compressed data are arranged into fixed-size blocks of
     * texels. $texture is the name of the texture object, and must not be a
     * buffer or multisample texture. The effective $target is the value of
     * {@see GL46::GL_TEXTURE_TARGET} for texture. $level and $pixels have
     * the same meaning as the corresponding arguments of
     * {@see GL46::glCompressedTexSubImage3D}. $bufSize indicates the size of
     * the buffer to receive the retrieved pixel data.
     *
     * For cube map textures, the behavior is as though
     * {@see GL46::glGetCompressedTexImage} were called once for each
     * requested face (selected by $zoffset and $depth, as described below)
     * with target corresponding to the requested texture cube map face as
     * indicated by the table presented below. $pixels is offset
     ...
     * to return. These arguments have the same meaning as for
     * {@see GL46::glCompressedTexSubImage3D}, though there are extra
     ...
     * map, array, and cube map array textures is the same as for
     * glGetTextureSubImage.
     *
     * The $xoffset, $yoffset, $zoffset offsets and $width, $height and
     * $depth sizes must be multiples of the values of
     * {@see GL46::GL_PACK_COMPRESSED_BLOCK_WIDTH},
     * {@see GL46::GL_PACK_COMPRESSED_BLOCK_HEIGHT}, and
     * {@see GL46::GL_PACK_COMPRESSED_BLOCK_DEPTH} respectively, unless
     * $offset is zero and the corresponding $size is the same as the texture
     * size in that dimension.
     *
     * Pixel storage modes are treated as for
     * {@see GL46::glGetCompressedTexSubImage}. The texel at ($xoffset,
     * $yoffset, $zoffset) will be stored at the location indicated by
     * $pixels and the current pixel packing parameters.
     *
    ...
    /**
     * {@see GL46::glGetTexLevelParameterfv},
     * {@see GL46::glGetTexLevelParameteriv},
     * {@see GL46::glGetTextureLevelParameterfv} and
     * {@see GL46::glGetTextureLevelParameteriv} return in $params texture
     * parameter values for a specific level-of-detail value, specified as
     * $level. For the first two functions, $target defines the target
     * texture, either {@see GL46::GL_TEXTURE_1D},
     * {@see GL46::GL_TEXTURE_2D}, {@see GL46::GL_TEXTURE_3D},
     * {@see GL46::GL_PROXY_TEXTURE_1D}, {@see GL46::GL_PROXY_TEXTURE_2D},
     * {@see GL46::GL_PROXY_TEXTURE_3D},
     * {@see GL46::GL_TEXTURE_CUBE_MAP_POSITIVE_X},
     * {@see GL46::GL_TEXTURE_CUBE_MAP_NEGATIVE_X},
     * {@see GL46::GL_TEXTURE_CUBE_MAP_POSITIVE_Y},
     * {@see GL46::GL_TEXTURE_CUBE_MAP_NEGATIVE_Y},
     * {@see GL46::GL_TEXTURE_CUBE_MAP_POSITIVE_Z},
     * {@see GL46::GL_TEXTURE_CUBE_MAP_NEGATIVE_Z}, or
     * {@see GL46::GL_PROXY_TEXTURE_CUBE_MAP}. The remaining two take a
     * $texture argument which specifies the name of the texture object.
     *
     * {@see GL46::GL_MAX_TEXTURE_SIZE}, and
     * {@see GL46::GL_MAX_3D_TEXTURE_SIZE} are not really descriptive enough.
     * It has to report the largest square texture image that can be
     * accommodated with mipmaps but a long skinny texture, or a texture
     * without mipmaps may easily fit in texture memory. The proxy targets
     * allow the user to more accurately query whether the GL can accommodate
     * a texture of a given configuration. If the texture cannot be
     * accommodated, the texture state variables, which may be queried with
     * {@see GL46::glGetTexLevelParameter} and
     * {@see GL46::glGetTextureLevelParameter}, are set to 0. If the texture
     * can be accommodated, the texture state values will be set as they
     ...
     *    {@see GL46::GL_NONE}, {@see GL46::GL_SIGNED_NORMALIZED},
     *    {@see GL46::GL_UNSIGNED_NORMALIZED}, {@see GL46::GL_FLOAT},
     *    {@see GL46::GL_INT}, and {@see GL46::GL_UNSIGNED_INT} may be returned
     *    to indicate signed normalized fixed-point, unsigned normalized
     ...
     *    single integer value, the number of unsigned bytes
     *    of the compressed texture image that would be returned from
     *    {@see GL46::glGetCompressedTexImage}.
     *
     *  - {@see GL46::GL_TEXTURE_BUFFER_OFFSET}: $params returns a single
     *    integer value, the offset into the data
     *    store of the buffer bound to a buffer texture.
     *    {@see GL46::glTexBufferRange}.
     *
     *  - {@see GL46::GL_TEXTURE_BUFFER_SIZE}: $params returns a single integer
     *    value, the size of the range of a
    ...
    /**
     * {@see GL46::glGetTexParameter} and {@see GL46::glGetTextureParameter}
     * return in $params the value or values of the texture parameter
     * specified as $pname. $target defines the target texture.
     * {@see GL46::GL_TEXTURE_1D}, {@see GL46::GL_TEXTURE_2D},
     * {@see GL46::GL_TEXTURE_3D}, {@see GL46::GL_TEXTURE_1D_ARRAY},
     * {@see GL46::GL_TEXTURE_2D_ARRAY}, {@see GL46::GL_TEXTURE_RECTANGLE},
     * {@see GL46::GL_TEXTURE_CUBE_MAP},
     * {@see GL46::GL_TEXTURE_CUBE_MAP_ARRAY},
     * {@see GL46::GL_TEXTURE_2D_MULTISAMPLE}, or
     * {@see GL46::GL_TEXTURE_2D_MULTISAMPLE_ARRAY} specify one-, two-, or
     * three-dimensional, one-dimensional array, two-dimensional array,
     * rectangle, cube-mapped or cube-mapped array, two-dimensional
     * multisample, or two-dimensional multisample array texturing,
     * respectively. $pname accepts the same symbols as
     * {@see GL46::glTexParameter}, with the same interpretations:
     *
     *  - {@see GL46::GL_DEPTH_STENCIL_TEXTURE_MODE}: Returns the single-value
     *    depth stencil texture mode, a symbolic
     *    constant. The initial value is {@see GL46::GL_DEPTH_COMPONENT}.
     *
     *  - {@see GL46::GL_TEXTURE_MAG_FILTER}: Returns the single-valued texture
     *    magnification filter, a symbolic
     *    constant. The initial value is {@see GL46::GL_LINEAR}.
     *
     *  - {@see GL46::GL_TEXTURE_MIN_FILTER}: Returns the single-valued texture
     *    minification filter, a symbolic
     *    constant. The initial value is {@see GL46::GL_NEAREST_MIPMAP_LINEAR}.
     *
     ...
     *    texture mipmap level. The initial value
     *    is 0.
     *
     ...
     *    a symbolic constant. The initial value is {@see GL46::GL_REPEAT}.
     *
     ...
     *    floating-point numbers that comprise the RGBA
     *    color of the texture border. Floating-point values are returned in the
     *    range    0 1  . Integer values are returned as a linear mapping of the
     ...
     *    comparison mode, a symbolic constant.
     *    The initial value is {@see GL46::GL_NONE}. See
     *    {@see GL46::glTexParameter}.
     *
     *  - {@see GL46::GL_TEXTURE_COMPARE_FUNC}: Returns a single-valued texture
     *    comparison function, a symbolic
     *    constant. The initial value is {@see GL46::GL_LEQUAL}. See
     *    {@see GL46::glTexParameter}.
     *
     *  - {@see GL46::GL_TEXTURE_VIEW_MIN_LEVEL}: Returns a single-valued base
     *    level of a texture view relative to its
     *    parent. The initial value is 0. See {@see GL46::glTextureView}.
     *
     *  - {@see GL46::GL_TEXTURE_VIEW_NUM_LEVELS}: Returns a single-valued
     *    number of levels of detail of a texture view.
     *    See {@see GL46::glTextureView}.
     *
     *  - {@see GL46::GL_TEXTURE_VIEW_MIN_LAYER}: Returns a single-valued first
     *    level of a texture array view relative
     *    to its parent. See {@see GL46::glTextureView}.
     *
     *  - {@see GL46::GL_TEXTURE_VIEW_NUM_LAYERS}: Returns a single-valued
     *    number of layers in a texture array view. See
     *    {@see GL46::glTextureView}.
     *
     *  - {@see GL46::GL_TEXTURE_IMMUTABLE_LEVELS}: Returns a single-valued
     *    number of immutable texture levels in a
     *    texture view. See {@see GL46::glTextureView}.
     *
     * In addition to the parameters that may be set with
     * {@see GL46::glTexParameter}, {@see GL46::glGetTexParameter} and
     * {@see GL46::glGetTextureParameter} accept the following read-only
     * parameters:
     *
     *  - {@see GL46::GL_IMAGE_FORMAT_COMPATIBILITY_TYPE}: Returns the matching
     *    criteria use for the texture when used as an
     *    image texture. Can return
     *    {@see GL46::GL_IMAGE_FORMAT_COMPATIBILITY_BY_SIZE},
     *    {@see GL46::GL_IMAGE_FORMAT_COMPATIBILITY_BY_CLASS} or
     *    {@see GL46::GL_NONE}.
     *
     *  - {@see GL46::GL_TEXTURE_IMMUTABLE_FORMAT}: Returns non-zero if the
     *    texture has an immutable format. Textures
     *    become immutable if their storage is specified with
     *    {@see GL46::glTexStorage1D}, {@see GL46::glTexStorage2D} or
     *    {@see GL46::glTexStorage3D}. The initial value is
     *    {@see GL46::GL_FALSE}.
     *
     *  - {@see GL46::GL_TEXTURE_TARGET}: Returns the effective target of the
     *    texture object. For
     *    {@see GL46::glGetTex*Parameter} functions, this is the target
     *    parameter. For {@see GL46::glGetTextureParameter*}, it is the target
     *    to which the texture was initially bound when it was created, or the
     *    value of the target parameter to the call to
     *    {@see GL46::glCreateTextures} which created the texture.
     *
     * @see http://docs.gl/gl2/glGetTexLevelParameter
    ...
    /**
     * {@see GL46::glCreateVertexArrays} returns $n previously unused vertex
    ...
    /**
     * {@see GL46::glVertexArrayElementBuffer} binds a buffer object with id
     * $buffer to the element array buffer bind point of a vertex array
     * object with id $vaobj. If $buffer is zero, any existing element array
    ...
    /**
     * {@see GL46::glVertexAttribBinding} and
     * {@see GL46::glVertexArrayAttribBinding} establishes an association
     * between the generic vertex attribute of a vertex array object whose
     * index is given by $attribindex, and a vertex buffer binding whose
     * index is given by $bindingindex. For
     * {@see GL46::glVertexAttribBinding}, the vertex array object affected
     * is that currently bound. For {@see GL46::glVertexArrayAttribBinding},
     * $vaobj is the name of the vertex array object.
     *
     * $attribindex must be less than the value of
     * {@see GL46::GL_MAX_VERTEX_ATTRIBS} and $bindingindex must be less than
     * the value of {@see GL46::GL_MAX_VERTEX_ATTRIB_BINDINGS}.
     *
    ...
    /**
     * {@see GL46::glVertexAttribFormat}, {@see GL46::glVertexAttribIFormat}
     * and {@see GL46::glVertexAttribLFormat}, as well as
     * {@see GL46::glVertexArrayAttribFormat},
     * {@see GL46::glVertexArrayAttribIFormat} and
     * {@see GL46::glVertexArrayAttribLFormat} specify the organization of
     * data in vertex arrays. The first three calls operate on the bound
     * vertex array object, whereas the last three ones modify the state of a
     * vertex array object with ID $vaobj. $attribindex specifies the index
     * of the generic vertex attribute array whose data layout is being
     * described, and must be less than the value of
     * {@see GL46::GL_MAX_VERTEX_ATTRIBS}.
     *
     * $size determines the number of components per vertex are allocated to
     * the specified attribute and must be 1, 2, 3 or 4. $type indicates the
     * type of the data. If $type is one of {@see GL46::GL_BYTE},
     * {@see GL46::GL_SHORT}, {@see GL46::GL_INT}, {@see GL46::GL_FIXED},
     * {@see GL46::GL_FLOAT}, {@see GL46::GL_HALF_FLOAT}, and
     * {@see GL46::GL_DOUBLE} indicate types `GLbyte`, `GLshort`, `GLint`,
     * `GLfixed`, `GLfloat`, `GLhalf`, and `GLdouble`, respectively; the
     * values {@see GL46::GL_UNSIGNED_BYTE}, {@see GL46::GL_UNSIGNED_SHORT},
     * and {@see GL46::GL_UNSIGNED_INT} indicate types `GLubyte`, `GLushort`,
     * and `GLuint`, respectively; the values
     * {@see GL46::GL_INT_2_10_10_10_REV} and
     * {@see GL46::GL_UNSIGNED_INT_2_10_10_10_REV} indicating respectively
     * four signed or unsigned elements packed into a single `GLuint`; and
     * the value {@see GL46::GL_UNSIGNED_INT_10F_11F_11F_REV} indicating
     * three floating point values packed into a single `GLuint`.
     *
     * {@see GL46::glVertexAttribLFormat} and
     * {@see GL46::glVertexArrayAttribLFormat} is used to specify layout for
     * data associated with a generic attribute variable declared as 64-bit
     * double precision components. For {@see GL46::glVertexAttribLFormat}
     * and {@see GL46::glVertexArrayAttribLFormat}, $type must be
     * {@see GL46::GL_DOUBLE}. In contrast to
     * {@see GL46::glVertexAttribFormat} or
     * {@see GL46::glVertexArrayAttribFormat}, which will cause data declared
     * as {@see GL46::GL_DOUBLE} to be converted to 32-bit representation,
     * {@see GL46::glVertexAttribLFormat} and
     * {@see GL46::glVertexArrayAttribLFormat} cause such data to be left in
     * its natural, 64-bit representation.
     *
     ...
     *
     * {@see GL46::glVertexAttribFormat} and
     * {@see GL46::glVertexArrayAttribFormat} should be used to describe
     * vertex attribute layout for floating-point vertex attributes,
     * {@see GL46::glVertexAttribIFormat} and
     * {@see GL46::glVertexArrayAttribIFormat} should be used to describe
     * vertex attribute layout for integer vertex attribute, and
     * {@see GL46::glVertexAttribLFormat} and
     * {@see GL46::glVertexArrayAttribLFormat} should be used to describe the
     * layout for 64-bit vertex attributes. Data for an array specified by
     * {@see GL46::glVertexAttribIFormat} and
     * {@see GL46::glVertexArrayAttribIFormat} will always be left as integer
    ...
    /**
     * {@see GL46::glVertexBindingDivisor} and
     * {@see GL46::glVertexArrayBindingDivisor} modify the rate at which
     * generic vertex attributes advance when rendering multiple instances of
     * primitives in a single draw command. If $divisor is zero, the
     * attributes using the buffer bound to $bindingindex advance once per
     * vertex. If $divisor is non-zero, the attributes advance once per
     * $divisor instances of the set(s) of vertices being rendered. An
     * attribute is referred to as instanced if the corresponding $divisor
     * value is non-zero.
     *
     * {@see GL46::glVertexBindingDivisor} uses currently bound vertex array
     * object , whereas {@see GL46::glVertexArrayBindingDivisor} updates
     * state of the vertex array object with ID $vaobj.
     *
     ...
     *
     * {@see GL46::glGetVertexArrayiv} can be used to retrieve ID of a buffer
     * object that will be bound to the {@see GL46::GL_ELEMENT_ARRAY_BUFFER}
     * binding point whenever the queried vertex array object is bound to the
     * rendering context. The binding can be changed for an active vertex
     * array object with a {@see GL46::glBindBuffer} call.
     *
    ...
    /**
     * {@see GL46::glGetVertexArrayIndexediv} and
     * {@see GL46::glGetVertexArrayIndexed64iv} provide a way of querying
     * parameters of an attribute at an user-specified index of a vertex
     ...
     * {@see GL46::glGetVertexArrayIndexediv} for each of the attributes
     * defined for a vertex array object:
     *
     *   {@see GL46::GL_VERTEX_ATTRIB_ARRAY_ENABLED}: $param returns a value
     ...
     * {@see GL46::GL_FALSE}.   {@see GL46::GL_VERTEX_ATTRIB_ARRAY_SIZE}:
     ...
     * is 4.   {@see GL46::GL_VERTEX_ATTRIB_ARRAY_STRIDE}: $param returns a
     * single value, the array stride for (number of bytes between successive
     * elements in) the vertex attribute array for index. A value of 0
     * indicates the array elements are stored sequentially in memory. The
     * initial value is 0.   {@see GL46::GL_VERTEX_ATTRIB_ARRAY_TYPE}: $param
     * returns a single value, a symbolic constant indicating the array type
     * for the vertex attribute array for index. Possible values are
     * {@see GL46::GL_BYTE}, {@see GL46::GL_DOUBLE}, {@see GL46::GL_FIXED},
     * {@see GL46::GL_FLOAT}, {@see GL46::GL_HALF_FLOAT},
     * {@see GL46::GL_INT}, {@see GL46::GL_INT_2_10_10_10_REV},
     * {@see GL46::GL_SHORT}, {@see GL46::GL_UNSIGNED_BYTE},
     * {@see GL46::GL_UNSIGNED_SHORT}, {@see GL46::GL_UNSIGNED_INT},
     * {@see GL46::GL_UNSIGNED_INT_2_10_10_10_REV}, and
     * {@see GL46::GL_UNSIGNED_INT_10F_11F_11F_REV} . The initial value is
     * {@see GL46::GL_FLOAT}.
     * {@see GL46::GL_VERTEX_ATTRIB_ARRAY_NORMALIZED}: $param returns a
     * single value that is non-zero (true) if fixed-point data types for the
     * vertex attribute array indicated by index are normalized when they are
     * converted to floating-point, and 0 (false) otherwise. The initial
     * value is {@see GL46::GL_FALSE}.
     * {@see GL46::GL_VERTEX_ATTRIB_ARRAY_INTEGER}: $param returns a single
     ...
     * unconverted double, and 0 (false) otherwise. The initial value is 0
     * ({@see GL46::GL_FALSE}).
     * {@see GL46::GL_VERTEX_ATTRIB_ARRAY_DIVISOR}: $param returns a single
     * value that is the frequency divisor used for instanced rendering. See
     * glVertexAttribDivisor. The initial value is 0.
     * {@see GL46::GL_VERTEX_ATTRIB_RELATIVE_OFFSET}: $param returns a single
     * value that is the byte offset of the first element relative to the
     * start of the vertex buffer binding specified attribute fetches from.
     * The initial value is 0.  {@see GL46::glGetVertexArrayIndexed64iv} can
     * be used to retrieve {@see GL46::GL_VERTEX_BINDING_OFFSET} parameter
     * value for any of the attributes defined for a vertex array object.
     * When $pname is set to {@see GL46::GL_VERTEX_BINDING_OFFSET}, $param
     * returns a single value that is the byte offset of the first element in
     * the bound buffer's data store. The initial value for this parameter is
     * 0.
     *
    ...
    /**
     * {@see GL46::glCreateSamplers} returns $n previously unused sampler
     * names in $samplers, each representing a new sampler object initialized
    ...
    /**
     * {@see GL46::glCreateProgramPipelines} returns $n previously unused
     * program pipeline names in $pipelines, each representing a new program
    ...
    /**
     * {@see GL46::glCreateQueries} returns $n previously unused query object
     * names in $ids, each representing a new query object with the specified
     * $target.
     *
     * $target may be one of {@see GL46::GL_SAMPLES_PASSED},
     * {@see GL46::GL_ANY_SAMPLES_PASSED},
     * {@see GL46::GL_ANY_SAMPLES_PASSED_CONSERVATIVE},
     * {@see GL46::GL_TIME_ELAPSED}, {@see GL46::GL_TIMESTAMP},
     * {@see GL46::GL_PRIMITIVES_GENERATED} or
     * {@see GL46::GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN}.
     *
     ...
     * specified by $id. {@see GL46::glGetQueryObject} returns in $params a
     * selected parameter of the query object specified by $id.
     * {@see GL46::glGetQueryBufferObject} returns in $buffer a selected
     ...
     *    the query object's passed
     *    samples counter. The initial value is 0.
     *
     *  - {@see GL46::GL_QUERY_RESULT_NO_WAIT}: If the result of the query is
     *    available (that is, a query of
     *    {@see GL46::GL_QUERY_RESULT_AVAILABLE} would return non-zero), then
     *    $params or $buffer returns the value of the query object's passed
     *    samples counter, otherwise, the data referred to by $params or $buffer
     *    is not modified. The initial value is 0.
     *
     *  - {@see GL46::GL_QUERY_RESULT_AVAILABLE}: $params or $buffer returns
     *    whether the passed samples counter is
     *    immediately available. If a delay would occur waiting for the query
     *    result, {@see GL46::GL_FALSE} is returned. Otherwise,
     *    {@see GL46::GL_TRUE} is returned, which also indicates that the
    ...
    /**
     * {@see GL46::glMemoryBarrier} defines a barrier ordering the memory
     * transactions issued prior to the command relative to those issued
     ...
     * synchronized with shader stores; the bits used in $barriers are as
     * follows:
     *
     *   {@see GL46::GL_VERTEX_ATTRIB_ARRAY_BARRIER_BIT}   If set, vertex
     * data sourced from buffer objects after the barrier will reflect data
     * written by shaders prior to the barrier. The set of buffer objects
     * affected by this bit is derived from the buffer object bindings used
     * for generic vertex attributes derived from the
     * {@see GL46::GL_VERTEX_ATTRIB_ARRAY_BUFFER} bindings.
     * {@see GL46::GL_ELEMENT_ARRAY_BARRIER_BIT}   If set, vertex array
     * indices sourced from buffer objects after the barrier will reflect
     * data written by shaders prior to the barrier. The buffer objects
     * affected by this bit are derived from the
     * {@see GL46::GL_ELEMENT_ARRAY_BUFFER} binding.
     * {@see GL46::GL_UNIFORM_BARRIER_BIT}   Shader uniforms sourced from
     * buffer objects after the barrier will reflect data written by shaders
     * prior to the barrier.     {@see GL46::GL_TEXTURE_FETCH_BARRIER_BIT}
     ...
     * {@see GL46::GL_SHADER_IMAGE_ACCESS_BARRIER_BIT}   Memory accesses
     * using shader image load, store, and atomic built-in functions issued
     * after the barrier will reflect data written by shaders prior to the
     * barrier. Additionally, image stores and atomics issued after the
     * barrier will not execute until all memory accesses (e.g., loads,
     * stores, texture fetches, vertex fetches) initiated prior to the
     * barrier complete.     {@see GL46::GL_COMMAND_BARRIER_BIT}   Command
     * data sourced from buffer objects by Draw*Indirect commands after the
     * barrier will reflect data written by shaders prior to the barrier. The
     * buffer objects affected by this bit are derived from the
     * {@see GL46::GL_DRAW_INDIRECT_BUFFER} binding.
     * {@see GL46::GL_PIXEL_BUFFER_BARRIER_BIT}   Reads and writes of buffer
     * objects via the {@see GL46::GL_PIXEL_PACK_BUFFER} and
     * {@see GL46::GL_PIXEL_UNPACK_BUFFER} bindings (via
     * {@see GL46::glReadPixels}, {@see GL46::glTexSubImage1D}, etc.) after
     ...
     *     {@see GL46::GL_TEXTURE_UPDATE_BARRIER_BIT}   Writes to a texture
     * via {@see GL46::glTex(Sub)Image*}, {@see GL46::glCopyTex(Sub)Image*},
     * {@see GL46::glCompressedTex(Sub)Image*}, and reads via
     * {@see GL46::glGetTexImage} after the barrier will reflect data written
     * by shaders prior to the barrier. Additionally, texture writes from
     * these commands issued after the barrier will not execute until all
     * shader writes initiated prior to the barrier complete.
     * {@see GL46::GL_BUFFER_UPDATE_BARRIER_BIT}   Reads or writes via
     * {@see GL46::glBufferSubData}, {@see GL46::glCopyBufferSubData}, or
     * {@see GL46::glGetBufferSubData}, or to buffer object memory mapped by
     * {@see GL46::glMapBuffer} or {@see GL46::glMapBufferRange} after the
     ...
     * {@see GL46::GL_FRAMEBUFFER_BARRIER_BIT}   Reads and writes via
     * framebuffer object attachments after the barrier will reflect data
     * written by shaders prior to the barrier. Additionally, framebuffer
     * writes issued after the barrier will wait on the completion of all
     * shader writes issued prior to the barrier.
     * {@see GL46::GL_TRANSFORM_FEEDBACK_BARRIER_BIT}   Writes via transform
     ...
     * {@see GL46::GL_ATOMIC_COUNTER_BARRIER_BIT}   Accesses to atomic
     * counters after the barrier will reflect writes prior to the barrier.
     *   {@see GL46::GL_SHADER_STORAGE_BARRIER_BIT}   Accesses to shader
     * storage blocks after the barrier will reflect writes prior to the
     * barrier.     {@see GL46::GL_QUERY_BUFFER_BARRIER_BIT}   Writes of
     * buffer objects via the {@see GL46::GL_QUERY_BUFFER} binding after the
     * barrier will reflect data written by shaders prior to the barrier.
     * Additionally, buffer object writes issued after the barrier will wait
     * on the completion of all shader writes initiated prior to the barrier.
     *
     * If $barriers is {@see GL46::GL_ALL_BARRIER_BITS}, shader memory
     * accesses will be synchronized relative to all the operations described
     * above.
     *
     ...
     * be separate caches for texture, vertex fetching, and one or more
     * caches for shader memory accesses. Implementations are not required to
     * keep these caches coherent with shader memory writes. Stores issued by
     * one invocation may not be immediately observable by other pipeline
     * stages or other shader invocations because the value stored may remain
     * in a cache local to the processor executing the store, or because data
     * overwritten by the store is still in a cache elsewhere in the system.
     * When {@see GL46::glMemoryBarrier} is called, the GL flushes and/or
     ...
     * the barrier.
     *
     * To allow for independent shader invocations to communicate by reads
     * and writes to a common memory address, image variables in the OpenGL
     * Shading Language may be declared as "coherent". Buffer object or
     * texture image memory accessed through such variables may be cached
     * only if caches are automatically updated due to stores issued by any
     * other shader invocation. If the same address is accessed using both
     * coherent and non-coherent variables, the accesses using variables
     * declared as coherent will observe the results stored using coherent
     * variables in other invocations. Using variables declared as "coherent"
     * guarantees only that the results of stores will be immediately visible
     * to shader invocations using similarly-declared variables; calling
     * {@see GL46::glMemoryBarrier} is required to ensure that the stores are
     ...
     * coherent variables or calling {@see GL46::MemoryBarrier}(). Updates to
     * the read-only data via API calls such as {@see GL46::glBufferSubData}
     * will invalidate shader caches implicitly as required.     Data that
     * are shared between shader invocations at a fine granularity (e.g.,
     * written by one invocation, consumed by another invocation) should use
     * coherent variables to read and write the shared data.     Data written
     * by one shader invocation and consumed by other shader invocations
     * launched as a result of its execution ("dependent invocations") should
     * use coherent variables in the producing shader invocation and call
     * {@see GL46::memoryBarrier}() after the last write. The consuming
     * shader invocation should also use coherent variables.     Data written
     * to image variables in one rendering pass and read by the shader in a
     * later pass need not use coherent variables or memoryBarrier(). Calling
     * {@see GL46::glMemoryBarrier} with the SHADER_IMAGE_ACCESS_BARRIER_BIT
     * set in $barriers between passes is necessary.     Data written by the
     * shader in one rendering pass and read by another mechanism (e.g.,
     * vertex or index buffer pulling) in a later pass need not use coherent
     * variables or {@see GL46::memoryBarrier}(). Calling
     * {@see GL46::glMemoryBarrier} with the appropriate bits set in
     ...
     * causes all context state to be lost and requires the application to
     * recreate all objects in the affected context.
     *
     * {@see GL46::glGetGraphicsResetStatus} can return one of the following
     * constants:
     *
     *  - {@see GL46::GL_NO_ERROR}: Indicates that the GL context has not been
     *    in a reset state since the
     *    last call.
     *
     *  - {@see GL46::GL_GUILTY_CONTEXT_RESET}: Indicates that a reset has been
     *    detected that is attributable to the
     *    current GL context.
     *
     *  - {@see GL46::GL_INNOCENT_CONTEXT_RESET}: Indicates a reset has been
     *    detected that is not attributable to the
     *    current GL context.
     *
     *  - {@see GL46::GL_UNKNOWN_CONTEXT_RESET}: Indicates a detected graphics
     *    reset whose cause is unknown.
     *
     * If a reset status other than {@see GL46::GL_NO_ERROR} is returned and
     * subsequent calls return {@see GL46::GL_NO_ERROR}, the context reset
     * was encountered and completed. If a reset status is repeatedly
     ...
     * and may be queried by calling {@see GL46::GetIntegerv} with the
     * symbolic constant {@see GL46::GL_RESET_NOTIFICATION_STRATEGY}.
     *
     * If the reset notification behavior is
     * {@see GL46::GL_NO_RESET_NOTIFICATION}, then the implementation will
     * never deliver notification of reset events, and
     * {@see GL46::glGetGraphicsResetStatus} will always return
     * {@see GL46::GL_NO_ERROR}.
     *
     * If the behavior is {@see GL46::GL_LOSE_CONTEXT_ON_RESET}, a graphics
     * reset will result in the loss of all context state, requiring the
     * recreation of all associated objects. In this case
     * {@see GL46::glGetGraphicsResetStatus} may return any of the values
     * described above.
     *
     ...
     * context.
     *
     * After a graphics reset has occurred on a context, subsequent GL
     * commands on that context (or any context which shares with that
     * context) will generate a {@see GL46::GL_CONTEXT_LOST} error. Such
     * commands will not have side effects (in particular, they will not
     * modify memory passed by pointer for query results), and will not block
     * indefinitely or cause termination of the application. There are two
     * important exceptions to this behavior:
     *
     *    {@see GL46::glGetError} and {@see GL46::glGetGraphicsResetStatus}
     * behave normally following a graphics reset, so that the application
     * can determine a reset has occurred, and when it is safe to destroy and
     * re-create the context.     Any commands which might cause a polling
     * application to block indefinitely will generate a
     * {@see GL46::GL_CONTEXT_LOST} error, but will also return a value
     * indicating completion to the application. Such commands include:
     * {@see GL46::glGetSynciv} with pname {@see GL46::GL_SYNC_STATUS}
     * ignores the other parameters and returns {@see GL46::GL_SIGNALED} in
     * $values.     {@see GL46::glGetQueryObjectuiv} with pname
     * {@see GL46::GL_QUERY_RESULT_AVAILABLE} ignores the other parameters
     * and returns {@see GL46::TRUE} in $params.
     *
    ...
    /**
     * {@see GL46::glGetTexImage}, {@see GL46::glGetnTexImage} and
     * {@see GL46::glGetTextureImage} functions return a texture image into
     * $pixels. For {@see GL46::glGetTexImage} and
     * {@see GL46::glGetnTexImage}, $target specifies whether the desired
     * texture image is one specified by {@see GL46::glTexImage1D}
     * ({@see GL46::GL_TEXTURE_1D}), {@see GL46::glTexImage2D}
     * ({@see GL46::GL_TEXTURE_1D_ARRAY}, {@see GL46::GL_TEXTURE_RECTANGLE},
     * {@see GL46::GL_TEXTURE_2D} or any of
     * {@see GL46::GL_TEXTURE_CUBE_MAP_*}), or {@see GL46::glTexImage3D}
     * ({@see GL46::GL_TEXTURE_2D_ARRAY}, {@see GL46::GL_TEXTURE_3D},
     * {@see GL46::GL_TEXTURE_CUBE_MAP_ARRAY}). For
     * {@see GL46::glGetTextureImage}, $texture specifies the texture object
     * name. In addition to types of textures accepted by
     * {@see GL46::glGetTexImage} and {@see GL46::glGetnTexImage}, the
     * function also accepts cube map texture objects (with effective target
     * {@see GL46::GL_TEXTURE_CUBE_MAP}). $level specifies the
     * level-of-detail number of the desired image. $format and $type specify
     * the format and type of the desired image array. See the reference page
     * for {@see GL46::glTexImage1D}  for a description of the acceptable
     * values for the $format and $type parameters, respectively. For
     * glGetnTexImage and glGetTextureImage functions, bufSize tells the size
     * of the buffer to receive the retrieved pixel data.
     * {@see GL46::glGetnTexImage} and {@see GL46::glGetTextureImage} do not
     * write more than $bufSize bytes into $pixels.
     *
     * If a non-zero named buffer object is bound to the
     * {@see GL46::GL_PIXEL_PACK_BUFFER} target (see
     * {@see GL46::glBindBuffer}) while a texture image is requested, $pixels
     * is treated as a byte offset into the buffer object's data store.
     *
     * To understand the operation of {@see GL46::glGetTexImage}, consider
     * the selected internal four-component texture image to be an RGBA color
     * buffer the size of the image. The semantics of
     * {@see GL46::glGetTexImage} are then identical to those of
     * {@see GL46::glReadPixels}, with the exception that no pixel transfer
     ...
     * {@see GL46::glGetTexLevelParameter} to determine the dimensions of the
     * internal texture image, then scale the required number of pixels by
     * the storage required for each pixel, based on $format and $type. Be
     * sure to take the pixel storage parameters into account, especially
     * {@see GL46::GL_PACK_ALIGNMENT}.
     *
     * If {@see GL46::glGetTextureImage} is used against a cube map texture
     * object, the texture is treated as a three-dimensional image of a depth
     * of 6, where the cube map faces are ordered as image layers, in an
    ...
    /**
     * {@see GL46::glReadPixels} and {@see GL46::glReadnPixels} return pixel
     * data from the frame buffer, starting with the pixel whose lower left
     * corner is at location ($x, $y), into client memory starting at
     * location $data. Several parameters control the processing of the pixel
     * data before it is placed into client memory. These parameters are set
     * with {@see GL46::glPixelStore}. This reference page describes the
     * effects on {@see GL46::glReadPixels} and {@see GL46::glReadnPixels} of
     * most, but not all of the parameters specified by these three commands.
     *
     * If a non-zero named buffer object is bound to the
     * {@see GL46::GL_PIXEL_PACK_BUFFER} target (see
     * {@see GL46::glBindBuffer}) while a block of pixels is requested, $data
     * is treated as a byte offset into the buffer object's data store rather
     * than a pointer to client memory.
     *
     * {@see GL46::glReadPixels} and {@see GL46::glReadnPixels} return values
     * from each pixel with lower left corner at     x + i   y + j    for
     * 0 &lt;= i &lt; width   and    0 &lt;= j &lt; height  . This pixel is
     ...
     *    buffer. Each component is
     *    converted to floating point such that the minimum depth value maps to
     *    0 and the maximum value maps to 1. Each component is clamped to the
     *    range    0 1  .
     *
     *  - {@see GL46::GL_DEPTH_STENCIL}: Values are taken from both the depth
     *    and stencil buffers. The $type
     *    parameter must be {@see GL46::GL_UNSIGNED_INT_24_8} or
     *    {@see GL46::GL_FLOAT_32_UNSIGNED_INT_24_8_REV}.
     *
     ...
     * as specified by $type. If $format is {@see GL46::GL_STENCIL_INDEX} and
     * $type is not {@see GL46::GL_FLOAT}, each index is masked with the mask
     * value given in the following table. If $type is {@see GL46::GL_FLOAT},
     * then each integer index is converted to single-precision
     * floating-point format.
     *
     * If $format is {@see GL46::GL_RED}, {@see GL46::GL_GREEN},
     * {@see GL46::GL_BLUE}, {@see GL46::GL_RGB}, {@see GL46::GL_BGR},
     * {@see GL46::GL_RGBA}, or {@see GL46::GL_BGRA} and $type is not
     * {@see GL46::GL_FLOAT}, each component is multiplied by the multiplier
     * shown in the following table. If type is {@see GL46::GL_FLOAT}, then
     ...
     * {@see GL46::GL_STENCIL_INDEX}, {@see GL46::GL_DEPTH_COMPONENT},
     * {@see GL46::GL_RED}, {@see GL46::GL_GREEN}, or {@see GL46::GL_BLUE}, a
     * single value is returned and the data for the ith pixel in the jth row
     * is placed in location     j  &amp;it; width + i  . {@see GL46::GL_RGB}
     * and {@see GL46::GL_BGR} return three values, {@see GL46::GL_RGBA} and
     * {@see GL46::GL_BGRA} return four values for each pixel, with all
     * values corresponding to a single pixel occupying contiguous space in
     * $data. Storage parameters set by {@see GL46::glPixelStore}, such as
     * {@see GL46::GL_PACK_LSB_FIRST} and {@see GL46::GL_PACK_SWAP_BYTES},
     * affect the way that data is written into memory. See
     * {@see GL46::glPixelStore}  for a description.
     *
     * {@see GL46::glReadnPixels} function will only handle the call if
     * $bufSize is at least of the size required to store the requested data.
     * Otherwise, it will generate a {@see GL46::GL_INVALID_OPERATION} error.
     *
