...
class GL44 extends GL43
{
    ...
    /**
     * {@see GL46::glClearTexImage} fills all an image contained in a texture
     * with an application supplied value. $texture must be the name of an
     ...
     * are interpreted as they are for {@see GL46::glTexImage3D}. Textures
     * with a base internal format of {@see GL46::GL_DEPTH_COMPONENT},
     * {@see GL46::GL_STENCIL_INDEX}, or {@see GL46::GL_DEPTH_STENCIL}
     ...
     * defined by any of the {@see GL46::glTexImage*},
     * {@see GL46::glTexStorage*} or {@see GL46::glCopyTexImage*} commands),
     * and then used to fill the specified range of the destination texture
     * level. If $data is {@see GL46::NULL}, then the pointer is ignored and
     * the sub-range of the texture image is filled with zeros. If texture is
     * a multisample texture, all the samples in a texel are cleared to the
    ...
    /**
     * {@see GL46::glClearTexSubImage} fills all or part of an image
     * contained in a texture with an application supplied value. $texture
     * must be the name of an existing texture. Further, $texture may not be
     * the name of a buffer texture, nor may its internal format be
     * compressed.
     *
     ...
     * rectangular subregion of the texel array.
     *
     * For one-dimensional array textures, $yoffset is interpreted as the
     * first layer to be cleared and $height is the number of layers to
     * clear. For two-dimensional array textures, $zoffset is interpreted as
     * the first layer to be cleared and $depth is the number of layers to
     * clear. Cube map textures are treated as an array of six slices in the
     * z-dimension, where the value of $zoffset is interpreted as specifying
     * the cube map face for the corresponding layer and $depth is the number
     * of faces to clear. For cube map array textures, $zoffset is the first
     * layer-face to clear, and $depth is the number of layer-faces to clear.
     * Each layer-face is translated into an array layer and a cube map face
     * as described in the OpenGL Specification.
     *
     * Negative values of $xoffset, $yoffset, and $zoffset correspond to the
     * coordinates of border texels. Taking  ws ,  hs ,  ds ,  wb ,  hb , and
     *  db  to be the specified $width, $height, $depth, and the border
     * width, border height, and border depth of the texel array and taking
     * x ,  y ,  z ,  w ,  h , and  d  to be the $xoffset, $yoffset,
     * $zoffset, $width, $height, and $depth argument values, any of the
     * following relationships generates a {@see GL46::GL_INVALID_OPERATION}
     * error:      x &lt;  w b          x + w &gt;  w s  -  w b          y
     * &lt; -  h b          y + h &gt;  h s  -  h b          z &lt; -  d b
     *       z + d &gt;  d s  -  d b
     *
     * For texture types that do not have certain dimensions, this command
     * treats those dimensions as having a size of 1. For example, to clear a
     * portion of a two-dimensional texture, use $zoffset equal to zero and
     * $depth equal to one.
     *
     * $format and $type specify the format and type of the source data and
     * are interpreted as they are for {@see GL46::glTexImage3D}. Textures
    ...
    /**
     * {@see GL46::glBindBuffersBase} binds a set of $count buffer objects
     * whose names are given in the array $buffers to the $count consecutive
     * binding points starting from index $first of the array of targets
     * specified by $target. If $buffers is {@see GL46::NULL} then
     * {@see GL46::glBindBuffersBase} unbinds any buffers that are currently
     * bound to the referenced binding points. Assuming no errors are
     * generated, it is equivalent to the following pseudo-code, which calls
     * {@see GL46::glBindBufferBase}, with the exception that the non-indexed
     ...
     * invalid, the state for that buffer binding index will not be changed
     * and an error will be generated. However, the state for other buffer
     * binding indices referenced by the command will still be updated.
     *
    ...
    /**
     * {@see GL46::glBindBuffersRange} binds a set of $count ranges from
     * buffer objects whose names are given in the array $buffers to the
     * $count consecutive binding points starting from index $first of the
     * array of targets specified by $target. $offsets specifies the address
     * of an array containing $count starting offsets within the buffers, and
     * $sizes specifies the address of an array of $count sizes of the
     * ranges. If $buffers is {@see GL46::NULL} then $offsets and $sizes are
     * ignored and {@see GL46::glBindBuffersRange} unbinds any buffers that
     * are currently bound to the referenced binding points. Assuming no
     * errors are generated, it is equivalent to the following pseudo-code,
     * which calls {@see GL46::glBindBufferRange}, with the exception that
     * the non-indexed $target is not changed by
     * {@see GL46::glBindBuffersRange}:
     *
     * <code>
     *     for (i = 0; i < count; i++) {
     *         if (buffers != NULL) {
    ...
    /**
     * {@see GL46::glBindTextures} binds an array of existing texture objects
     * to a specified number of consecutive texture units. $count specifies
     * the number of texture objects whose names are stored in the array
     ...
     * {@see GL46::NULL} then it is as if an appropriately sized array
     * containing only zeros had been specified.
     *
     * With the exception that the active texture selector maintains its
     * current value, {@see GL46::glBindTextures} is equivalent to the
    ...
    /**
     * {@see GL46::glBindSamplers} binds samplers from an array of existing
     * sampler objects to a specified number of consecutive sampler units.
     * $count specifies the number of sampler objects whose names are stored
     * in the array $samplers. That number of sampler names is read from the
     * array and bound to the $count consecutive sampler units starting from
     * $first.
     *
     ...
     * corresponding sampler unit. If $samplers is {@see GL46::NULL} then it
     * is as if an appropriately sized array containing only zeros had been
     * specified.
     *
     * {@see GL46::glBindSamplers} is equivalent to the following pseudo
     * code:
     *
     * <code>
     *     for (i = 0; i < count; i++) {
    ...
    /**
     * {@see GL46::glBindImageTextures} binds images from an array of
     * existing texture objects to a specified number of consecutive image
     * units. $count specifies the number of texture objects whose names are
     * stored in the array $textures. That number of texture names are read
     * from the array and bound to the $count consecutive texture units
     * starting from $first. If the name zero appears in the $textures array,
     * any existing binding to the image unit is reset. Any non-zero entry in
     * $textures must be the name of an existing texture object. When a
     * non-zero entry in $textures is present, the image at level zero is
     * bound, the binding is considered layered, with the first layer set to
     * zero, and the image is bound for read-write access. The image unit
     * format parameter is taken from the internal format of the image at
     * level zero of the texture object. For cube map textures, the internal
     * format of the positive X image of level zero is used. If $textures is
     * {@see GL46::NULL} then it is as if an appropriately sized array
     * containing only zeros had been specified.
     *
     * {@see GL46::glBindImageTextures} is equivalent to the following pseudo
     * code:
     *
     * <code>
     *     for (i = 0; i < count; i++) {
    ...
    /**
     * {@see GL46::glBindVertexBuffers} and
     * {@see GL46::glVertexArrayVertexBuffers} bind storage from an array of
     * existing buffer objects to a specified number of consecutive vertex
     * buffer binding points units in a vertex array object. For
     * {@see GL46::glBindVertexBuffers}, the vertex array object is the
     * currently bound vertex array object. For
     * {@see GL46::glVertexArrayVertexBuffers}, $vaobj is the name of the
     * vertex array object.
     *
     ...
     * $strides specify arrays of $count values indicating the offset of the
     * first element and stride between elements in each buffer,
     * respectively. If $buffers is NULL, each affected vertex buffer binding
     * point from $first$ through $first + count - 1$ will be reset to have
     * no bound buffer object. In this case, the offsets and strides
     * associated with the binding points are set to default values, ignoring
     * $offsets and $strides.
     *
     * {@see GL46::glBindVertexBuffers} is equivalent (assuming no errors are
     * generated) to:
     *
     * <code>
     *     for (i = 0; i < count; i++) {
     ...
     *         } else {
     *             glBindVertexBuffer(first + i, buffers[i], offsets[i],
     * strides[i]);
     *         }
     *     }
     * </code>
     ...
     *
     * {@see GL46::glVertexArrayVertexBuffers} is equivalent to the
     * pseudocode above, but replacing {@see GL46::glBindVertexBuffers}(args)
     * with {@see GL46::glVertexArrayVertexBuffers}(vaobj, args).
     *
