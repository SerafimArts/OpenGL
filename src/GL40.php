...
class GL40 extends GL33
{
    ...
    /**
     * {@see GL46::glMinSampleShading} specifies the rate at which samples
     * are shaded within a covered pixel. Sample-rate shading is enabled by
     * calling {@see GL46::glEnable} with the parameter
     * {@see GL46::GL_SAMPLE_SHADING}. If {@see GL46::GL_MULTISAMPLE} or
     * {@see GL46::GL_SAMPLE_SHADING} is disabled, sample shading has no
     * effect. Otherwise, an implementation must provide at least as many
     * unique color values for each covered fragment as specified by $value
     * times $samples where $samples is the value of {@see GL46::GL_SAMPLES}
     * for the current framebuffer. At least 1 sample for each covered
     ...
     * @return void
     */
    public function glMinSampleShading($value): void
    {
     ...
     * @return void
     */
    public function glBlendEquationi($buf, $mode): void
    {
        $buf = $buf instanceof \FFI\CData ? $buf->cdata : $buf;
        $mode = $mode instanceof \FFI\CData ? $mode->cdata : $mode;

        assert(Assert::uint16($buf), 'Argument $buf must be a C-like GLuint, but incompatible or overflow value given');
     ...
     * @return void
     */
    public function glBlendEquationSeparatei($buf, $modeRGB, $modeAlpha): void
    {
        $buf = $buf instanceof \FFI\CData ? $buf->cdata : $buf;
     ...
     *
     * {@see GL46::glBlendFunc} defines the operation of blending for all
     * draw buffers when it is enabled. {@see GL46::glBlendFunci} defines the
     * operation of blending for a single draw buffer specified by $buf when
     * enabled for that draw buffer. $sfactor specifies which method is used
     * to scale the source color components. $dfactor specifies which method
     * is used to scale the destination color components. Both parameters
     * must be one of the following symbolic constants: {@see GL46::GL_ZERO},
     * {@see GL46::GL_ONE}, {@see GL46::GL_SRC_COLOR},
     * {@see GL46::GL_ONE_MINUS_SRC_COLOR}, {@see GL46::GL_DST_COLOR},
     * {@see GL46::GL_ONE_MINUS_DST_COLOR}, {@see GL46::GL_SRC_ALPHA},
     * {@see GL46::GL_ONE_MINUS_SRC_ALPHA}, {@see GL46::GL_DST_ALPHA},
     * {@see GL46::GL_ONE_MINUS_DST_ALPHA}, {@see GL46::GL_CONSTANT_COLOR},
     * {@see GL46::GL_ONE_MINUS_CONSTANT_COLOR},
     * {@see GL46::GL_CONSTANT_ALPHA},
     * {@see GL46::GL_ONE_MINUS_CONSTANT_ALPHA},
     * {@see GL46::GL_SRC_ALPHA_SATURATE}, {@see GL46::GL_SRC1_COLOR},
     * {@see GL46::GL_ONE_MINUS_SRC1_COLOR}, {@see GL46::GL_SRC1_ALPHA}, and
     * {@see GL46::GL_ONE_MINUS_SRC1_ALPHA}. The possible methods are
     ...
     * subsequent equations, first source, second source and destination
     * color components are referred to as    R s0  G s0  B s0  A s0   ,    R
     * s1  G s1  B s1  A s1    and    R d  G d  B d  A d   , respectively.
     * The color specified by {@see GL46::glBlendColor} is referred to as
     ...
     * B  s A    and    d R  d G  d B  d A   . The scale factors described in
     * the table, denoted    f R  f G  f B  f A   , represent either source
     ...
     * $sfactor is {@see GL46::GL_SRC_ALPHA}, $dfactor is
     ...
     * @return void
     */
    public function glBlendFunci($buf, $src, $dst): void
    {
        $buf = $buf instanceof \FFI\CData ? $buf->cdata : $buf;
        ...
        assert(Assert::uint16($dst), 'Argument $dst must be a C-like GLenum, but incompatible or overflow value given');
     ...
     * @return void
     */
    public function glBlendFuncSeparatei($buf, $srcRGB, $dstRGB, $srcAlpha, $dstAlpha): void
    {
        $buf = $buf instanceof \FFI\CData ? $buf->cdata : $buf;
        ...
        assert(Assert::uint16($buf), 'Argument $buf must be a C-like GLuint, but incompatible or overflow value given');
        assert(Assert::uint16($srcRGB), 'Argument $srcRGB must be a C-like GLenum, but incompatible or overflow value given');
        assert(Assert::uint16($dstRGB), 'Argument $dstRGB must be a C-like GLenum, but incompatible or overflow value given');
        assert(Assert::uint16($srcAlpha), 'Argument $srcAlpha must be a C-like GLenum, but incompatible or overflow value given');
        assert(Assert::uint16($dstAlpha), 'Argument $dstAlpha must be a C-like GLenum, but incompatible or overflow value given');
    ...
    /**
     * {@see GL46::glDrawArraysIndirect} specifies multiple geometric
     * primitives with very few subroutine calls.
     * {@see GL46::glDrawArraysIndirect} behaves similarly to
     * {@see GL46::glDrawArraysInstancedBaseInstance}, execept that the
     * parameters to {@see GL46::glDrawArraysInstancedBaseInstance} are
     ...
     *
     *     const DrawArraysIndirectCommand *cmd = (const
     ...
     * binding at the time of a call to {@see GL46::glDrawArraysIndirect},
     ...
     * {@see GL46::glDrawArraysIndirect} have an unspecified value after
     * {@see GL46::glDrawArraysIndirect} returns. Attributes that aren't
     ...
     * @return void
     */
    public function glDrawArraysIndirect($mode, ?\FFI\CData $indirect): void
    {
        $mode = $mode instanceof \FFI\CData ? $mode->cdata : $mode;
    ...
    /**
     * {@see GL46::glDrawElementsIndirect} specifies multiple indexed
     * geometric primitives with very few subroutine calls.
     * {@see GL46::glDrawElementsIndirect} behaves similarly to
     ...
     * @return void
     */
    public function glDrawElementsIndirect($mode, $type, ?\FFI\CData $indirect): void
    {
        $mode = $mode instanceof \FFI\CData ? $mode->cdata : $mode;
     ...
     * @return void
     */
    public function glUniform1d($location, $x): void
    {
     ...
     * @return void
     */
    public function glUniform2d($location, $x, $y): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $x = $x instanceof \FFI\CData ? $x->cdata : $x;
     ...
     * @return void
     */
    public function glUniform3d($location, $x, $y, $z): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $x = $x instanceof \FFI\CData ? $x->cdata : $x;
        $y = $y instanceof \FFI\CData ? $y->cdata : $y;
     ...
     * @return void
     */
    public function glUniform4d($location, $x, $y, $z, $w): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $x = $x instanceof \FFI\CData ? $x->cdata : $x;
        $y = $y instanceof \FFI\CData ? $y->cdata : $y;
     ...
     * @return void
     */
    public function glUniform1dv($location, $count, ?\FFI\CData $value): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
     ...
     * @return void
     */
    public function glUniform2dv($location, $count, ?\FFI\CData $value): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
     ...
     * @return void
     */
    public function glUniform3dv($location, $count, ?\FFI\CData $value): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
     ...
     * @return void
     */
    public function glUniform4dv($location, $count, ?\FFI\CData $value): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
     ...
     * @return void
     */
    public function glUniformMatrix2dv($location, $count, $transpose, ?\FFI\CData $value): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
     ...
     * @return void
     */
    public function glUniformMatrix3dv($location, $count, $transpose, ?\FFI\CData $value): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
        $transpose = $transpose instanceof \FFI\CData ? $transpose->cdata : $transpose;
     ...
     * @return void
     */
    public function glUniformMatrix4dv($location, $count, $transpose, ?\FFI\CData $value): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
        $transpose = $transpose instanceof \FFI\CData ? $transpose->cdata : $transpose;
     ...
     * @return void
     */
    public function glUniformMatrix2x3dv($location, $count, $transpose, ?\FFI\CData $value): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
        $transpose = $transpose instanceof \FFI\CData ? $transpose->cdata : $transpose;
     ...
     * @return void
     */
    public function glUniformMatrix2x4dv($location, $count, $transpose, ?\FFI\CData $value): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
        $transpose = $transpose instanceof \FFI\CData ? $transpose->cdata : $transpose;
     ...
     * @return void
     */
    public function glUniformMatrix3x2dv($location, $count, $transpose, ?\FFI\CData $value): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
        $transpose = $transpose instanceof \FFI\CData ? $transpose->cdata : $transpose;
     ...
     * @return void
     */
    public function glUniformMatrix3x4dv($location, $count, $transpose, ?\FFI\CData $value): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
        $transpose = $transpose instanceof \FFI\CData ? $transpose->cdata : $transpose;
     ...
     * @return void
     */
    public function glUniformMatrix4x2dv($location, $count, $transpose, ?\FFI\CData $value): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
        $transpose = $transpose instanceof \FFI\CData ? $transpose->cdata : $transpose;
     ...
     * @return void
     */
    public function glUniformMatrix4x3dv($location, $count, $transpose, ?\FFI\CData $value): void
    {
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
        $transpose = $transpose instanceof \FFI\CData ? $transpose->cdata : $transpose;
     ...
     * @return void
     */
    public function glGetUniformdv($program, $location, ?\FFI\CData $params): void
    {
    ...
    /**
     * {@see GL46::glGetSubroutineUniformLocation} returns the location of
     * the subroutine uniform variable $name in the shader stage of type
     * $shadertype attached to $program, with behavior otherwise identical to
     ...
     * value of {@see GL46::GL_ACTIVE_SUBROUTINE_LOCATIONS} minus one will be
     * returned. Subroutine locations are assigned using consecutive integers
     * in the range from zero to the value of
     * {@see GL46::GL_ACTIVE_SUBROUTINE_LOCATIONS} minus one for the shader
     * stage. For active subroutine uniforms declared as arrays, the declared
     ...
     */
    public function glGetSubroutineUniformLocation($program, $shadertype, ?\FFI\CData $name): int
    {
        $program = $program instanceof \FFI\CData ? $program->cdata : $program;
    ...
    /**
     * {@see GL46::glGetSubroutineIndex} returns the index of a subroutine
     * uniform within a shader stage attached to a program object. $program
     * contains the name of the program to which the shader is attached.
     * $shadertype specifies the stage from which to query shader subroutine
     * index. $name contains the null-terminated name of the subroutine
     * uniform whose name to query.
     *
     * If $name is not the name of a subroutine uniform in the shader stage,
     * {@see GL46::GL_INVALID_INDEX} is returned, but no error is generated.
     * If $name is the name of a subroutine uniform in the shader stage, a
     * value between zero and the value of {@see GL46::GL_ACTIVE_SUBROUTINES}
     * minus one will be returned. Subroutine indices are assigned using
     ...
     * @return int|\FFI\CData|\FFI\CInt
     */
    public function glGetSubroutineIndex($program, $shadertype, ?\FFI\CData $name): int
    {
        $program = $program instanceof \FFI\CData ? $program->cdata : $program;
        $shadertype = $shadertype instanceof \FFI\CData ? $shadertype->cdata : $shadertype;
    ...
    /**
     * {@see GL46::glGetActiveSubroutineUniform} queries a parameter of an
     * active shader subroutine uniform. $program contains the name of the
     * program containing the uniform. $shadertype specifies the stage which
     * the uniform location, given by $index, is valid. $index must be
     * between zero and the value of
     * {@see GL46::GL_ACTIVE_SUBROUTINE_UNIFORMS} minus one for the shader
     * stage.
     *
     * If $pname is {@see GL46::GL_NUM_COMPATIBLE_SUBROUTINES}, a single
     * integer indicating the number of subroutines that can be assigned to
     * the uniform is returned in $values.
     *
     * If $pname is {@see GL46::GL_COMPATIBLE_SUBROUTINES}, an array of
     * integers is returned in $values, with each integer specifying the
     * index of an active subroutine that can be assigned to the selected
     * subroutine uniform. The number of integers returned is the same as the
     * value returned for {@see GL46::GL_NUM_COMPATIBLE_SUBROUTINES}.
     *
     * If $pname is {@see GL46::GL_UNIFORM_SIZE}, a single integer is
     * returned in $values. If the selected subroutine uniform is an array,
     * the declared size of the array is returned; otherwise, one is
     * returned.
     *
     * If $pname is {@see GL46::GL_UNIFORM_NAME_LENGTH}, a single integer
     * specifying the length of the subroutine uniform name (including the
     ...
     * @return void
     */
    public function glGetActiveSubroutineUniformiv($program, $shadertype, $index, $pname, ?\FFI\CData $values): void
    {
        $program = $program instanceof \FFI\CData ? $program->cdata : $program;
        $shadertype = $shadertype instanceof \FFI\CData ? $shadertype->cdata : $shadertype;
    ...
    /**
     * {@see GL46::glGetActiveSubroutineUniformName} retrieves the name of an
     * active shader subroutine uniform. $program contains the name of the
     * program containing the uniform. $shadertype specifies the stage for
     * which the uniform location, given by $index, is valid. $index must be
     * between zero and the value of
     * {@see GL46::GL_ACTIVE_SUBROUTINE_UNIFORMS} minus one for the shader
     * stage.
     *
     ...
     * terminator is returned in $length. If $length is {@see GL46::NULL}, no
     ...
     * {@see GL46::GL_ACTIVE_SUBROUTINE_UNIFORM_MAX_LENGTH}, which can be
     * queried with {@see GL46::glGetProgramStage}.
     *
     ...
     * @return void
     */
    public function glGetActiveSubroutineUniformName($program, $shadertype, $index, $bufsize, ?\FFI\CData $length, ?\FFI\CData $name): void
    {
        $program = $program instanceof \FFI\CData ? $program->cdata : $program;
        $shadertype = $shadertype instanceof \FFI\CData ? $shadertype->cdata : $shadertype;
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
    ...
    /**
     * {@see GL46::glGetActiveSubroutineName} queries the name of an active
     ...
     * not including the null-terminator, is returned in $length. If $length
     * is {@see GL46::NULL}, no length is returned. The maximum number of
     ...
     * @return void
     */
    public function glGetActiveSubroutineName($program, $shadertype, $index, $bufsize, ?\FFI\CData $length, ?\FFI\CData $name): void
    {
        $program = $program instanceof \FFI\CData ? $program->cdata : $program;
        $shadertype = $shadertype instanceof \FFI\CData ? $shadertype->cdata : $shadertype;
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
     ...
     * @return void
     */
    public function glUniformSubroutinesuiv($shadertype, $count, ?\FFI\CData $indices): void
    {
        $shadertype = $shadertype instanceof \FFI\CData ? $shadertype->cdata : $shadertype;
        $count = $count instanceof \FFI\CData ? $count->cdata : $count;
    ...
    /**
     * {@see GL46::glGetUniformSubroutine} retrieves the value of the
     * subroutine uniform at location $location for shader stage $shadertype
     * of the current program. $location must be less than the value of
     * {@see GL46::GL_ACTIVE_SUBROUTINE_UNIFORM_LOCATIONS} for the shader
     * currently in use at shader stage $shadertype. The value of the
     * subroutine uniform is returned in $values.
     *
     ...
     * @return void
     */
    public function glGetUniformSubroutineuiv($shadertype, $location, ?\FFI\CData $params): void
    {
        $shadertype = $shadertype instanceof \FFI\CData ? $shadertype->cdata : $shadertype;
        $location = $location instanceof \FFI\CData ? $location->cdata : $location;
    ...
    /**
     * {@see GL46::glGetProgramStage} queries a parameter of a shader stage
     ...
     * $values.
     *
     * If $pname is {@see GL46::GL_ACTIVE_SUBROUTINE_UNIFORMS}, the number of
     * active subroutine variables in the stage is returned in $values.
     *
     * If $pname is {@see GL46::GL_ACTIVE_SUBROUTINE_UNIFORM_LOCATIONS}, the
     * number of active subroutine variable locations in the stage is
     * returned in $values.
     *
     * If $pname is {@see GL46::GL_ACTIVE_SUBROUTINES}, the number of active
     * subroutines in the stage is returned in $values.
     *
     * If $pname is {@see GL46::GL_ACTIVE_SUBROUTINE_UNIFORM_MAX_LENGTH}, the
     * length of the longest subroutine uniform for the stage is returned in
     * $values.
     *
     * If $pname is {@see GL46::GL_ACTIVE_SUBROUTINE_MAX_LENGTH}, the length
     * of the longest subroutine name for the stage is returned in $values.
     ...
     * @return void
     */
    public function glGetProgramStageiv($program, $shadertype, $pname, ?\FFI\CData $values): void
    {
        $program = $program instanceof \FFI\CData ? $program->cdata : $program;
        $shadertype = $shadertype instanceof \FFI\CData ? $shadertype->cdata : $shadertype;
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
    ...
    /**
     * {@see GL46::glPatchParameter} specifies the parameters that will be
     * used for patch primitives. $pname specifies the parameter to modify
     * and must be either {@see GL46::GL_PATCH_VERTICES},
     * {@see GL46::GL_PATCH_DEFAULT_OUTER_LEVEL} or
     * {@see GL46::GL_PATCH_DEFAULT_INNER_LEVEL}. For
     * {@see GL46::glPatchParameteri}, $value specifies the new value for the
     * parameter specified by $pname. For {@see GL46::glPatchParameterfv},
     * $values specifies the address of an array containing the new values
     * for the parameter specified by $pname.
     *
     * When $pname is {@see GL46::GL_PATCH_VERTICES}, $value specifies the
     * number of vertices that will be used to make up a single patch
     * primitive. Patch primitives are consumed by the tessellation control
     * shader (if present) and subsequently used for tessellation. When
     * primitives are specified using {@see GL46::glDrawArrays} or a similar
     * function, each patch will be made from $parameter control points, each
     * represented by a vertex taken from the enabeld vertex arrays.
     * $parameter must be greater than zero, and less than or equal to the
     * value of {@see GL46::GL_MAX_PATCH_VERTICES}.
     *
     * When $pname is {@see GL46::GL_PATCH_DEFAULT_OUTER_LEVEL} or
     * {@see GL46::GL_PATCH_DEFAULT_INNER_LEVEL}, $values contains the
     * address of an array contiaining the default outer or inner
     * tessellation levels, respectively, to be used when no tessellation
     ...
     * @return void
     */
    public function glPatchParameteri($pname, $value): void
    {
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
        $value = $value instanceof \FFI\CData ? $value->cdata : $value;
     ...
     * @return void
     */
    public function glPatchParameterfv($pname, ?\FFI\CData $values): void
    {
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
     ...
     * @return void
     */
    public function glBindTransformFeedback($target, $id): void
    {
    ...
    /**
     * {@see GL46::glDeleteTransformFeedbacks} deletes the $n transform
     * feedback objects whose names are stored in the array $ids. Unused
     * names in $ids are ignored, as is the name zero. After a transform
     * feedback object is deleted, its name is again unused and it has no
     * contents. If an active transform feedback object is deleted, its name
     ...
     * @return void
     */
    public function glDeleteTransformFeedbacks($n, ?\FFI\CData $ids): void
    {
    ...
    /**
     * {@see GL46::glGenTransformFeedbacks} returns $n previously unused
     * transform feedback object names in $ids. These names are marked as
     * used, for the purposes of {@see GL46::glGenTransformFeedbacks} only,
     * but they acquire transform feedback state only when they are first
     * bound.
     *
     ...
     * @return void
     */
    public function glGenTransformFeedbacks($n, ?\FFI\CData $ids): void
    {
        $n = $n instanceof \FFI\CData ? $n->cdata : $n;
    ...
    /**
     * {@see GL46::glIsTransformFeedback} returns {@see GL46::GL_TRUE} if $id
     * is currently the name of a transform feedback object. If $id is zero,
     * or if {@see GL46::id} is not the name of a transform feedback object,
     * or if an error occurs, {@see GL46::glIsTransformFeedback} returns
     * {@see GL46::GL_FALSE}. If $id is a name returned by
     * {@see GL46::glGenTransformFeedbacks}, but that has not yet been bound
     * through a call to {@see GL46::glBindTransformFeedback}, then the name
     * is not a transform feedback object and
     ...
     * @return int|\FFI\CData|\FFI\CInt
     */
    public function glIsTransformFeedback($id): int
    {
        $id = $id instanceof \FFI\CData ? $id->cdata : $id;
    ...
    /**
     * {@see GL46::glPauseTransformFeedback} pauses transform feedback
    ...
    /**
     * {@see GL46::glResumeTransformFeedback} resumes transform feedback
     * operations on the currently active transform feedback object. When
     * transform feedback operations are paused, transform feedback is still
     * considered active and changing most transform feedback state related
     * to the object results in an error. However, a new transform feedback
    ...
    /**
     * {@see GL46::glDrawTransformFeedback} draws primitives of a type
     * specified by $mode using a count retrieved from the transform feedback
     * specified by $id. Calling {@see GL46::glDrawTransformFeedback} is
     * equivalent to calling {@see GL46::glDrawArrays} with $mode as
     ...
     * @return void
     */
    public function glDrawTransformFeedback($mode, $id): void
    {
        $mode = $mode instanceof \FFI\CData ? $mode->cdata : $mode;
        $id = $id instanceof \FFI\CData ? $id->cdata : $id;
    ...
    /**
     * {@see GL46::glDrawTransformFeedbackStream} draws primitives of a type
     * specified by $mode using a count retrieved from the transform feedback
     * stream specified by $stream of the transform feedback object specified
     * by $id. Calling {@see GL46::glDrawTransformFeedbackStream} is
     * equivalent to calling {@see GL46::glDrawArrays} with $mode as
     * specified, $first set to zero, and $count set to the number of
     ...
     * @return void
     */
    public function glDrawTransformFeedbackStream($mode, $id, $stream): void
    {
        $mode = $mode instanceof \FFI\CData ? $mode->cdata : $mode;
        $id = $id instanceof \FFI\CData ? $id->cdata : $id;
    ...
    /**
     * {@see GL46::glBeginQueryIndexed} and  {@see GL46::glEndQueryIndexed}
     * delimit the boundaries of a query object. $query must be a name
     * previously returned from a call to {@see GL46::glGenQueries}. If a
     * query object with name $id does not yet exist it is created with the
     * type determined by $target. $target must be one of
     * {@see GL46::GL_SAMPLES_PASSED}, {@see GL46::GL_ANY_SAMPLES_PASSED},
     * {@see GL46::GL_PRIMITIVES_GENERATED},
     * {@see GL46::GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN}, or
     * {@see GL46::GL_TIME_ELAPSED}. The behavior of the query object depends
     * on its type and is as follows.
     *
     * $index specifies the index of the query target and must be between a
     * $target-specific maximum.
     *
     * If $target is {@see GL46::GL_SAMPLES_PASSED}, $id must be an unused
     * name, or the name of an existing occlusion query object. When
     * {@see GL46::glBeginQueryIndexed} is executed, the query object's
     * samples-passed counter is reset to 0. Subsequent rendering will
     * increment the counter for every sample that passes the depth test. If
     * the value of {@see GL46::GL_SAMPLE_BUFFERS} is 0, then the
     * samples-passed count is incremented by 1 for each fragment. If the
     * value of {@see GL46::GL_SAMPLE_BUFFERS} is 1, then the samples-passed
     * count is incremented by the number of samples whose coverage bit is
     * set. However, implementations, at their discression may instead
     * increase the samples-passed count by the value of
     * {@see GL46::GL_SAMPLES} if any sample in the fragment is covered. When
     * {@see GL46::glEndQueryIndexed} is executed, the samples-passed counter
     * is assigned to the query object's result value. This value can be
     * queried by calling {@see GL46::glGetQueryObject} with $pname
     * {@see GL46::GL_QUERY_RESULT}. When $target is
     * {@see GL46::GL_SAMPLES_PASSED}, $index must be zero.
     *
     * If $target is {@see GL46::GL_ANY_SAMPLES_PASSED}, $id must be an
     * unused name, or the name of an existing boolean occlusion query
     * object. When {@see GL46::glBeginQueryIndexed} is executed, the query
     * object's samples-passed flag is reset to {@see GL46::GL_FALSE}.
     * Subsequent rendering causes the flag to be set to {@see GL46::GL_TRUE}
     * if any sample passes the depth test. When
     * {@see GL46::glEndQueryIndexed} is executed, the samples-passed flag is
     * assigned to the query object's result value. This value can be queried
     * by calling {@see GL46::glGetQueryObject} with $pname
     * {@see GL46::GL_QUERY_RESULT}. When $target is
     * {@see GL46::GL_ANY_SAMPLES_PASSED}, $index must be zero.
     *
     * If $target is {@see GL46::GL_PRIMITIVES_GENERATED}, $id must be an
     * unused name, or the name of an existing primitive query object
     * previously bound to the {@see GL46::GL_PRIMITIVES_GENERATED} query
     * binding. When {@see GL46::glBeginQueryIndexed} is executed, the query
     * object's primitives-generated counter is reset to 0. Subsequent
     * rendering will increment the counter once for every vertex that is
     * emitted from the geometry shader to the stream given by $index, or
     * from the vertex shader if $index is zero and no geometry shader is
     * present. When {@see GL46::glEndQueryIndexed} is executed, the
     * primitives-generated counter for stream $index is assigned to the
     * query object's result value. This value can be queried by calling
     * {@see GL46::glGetQueryObject} with $pname
     * {@see GL46::GL_QUERY_RESULT}. When $target is
     * {@see GL46::GL_PRIMITIVES_GENERATED}, $index must be less than the
     * value of {@see GL46::GL_MAX_VERTEX_STREAMS}.
     *
     * If $target is {@see GL46::GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN},
     * $id must be an unused name, or the name of an existing primitive query
     * object previously bound to the
     * {@see GL46::GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN} query binding.
     * When {@see GL46::glBeginQueryIndexed} is executed, the query object's
     ...
     * call to {@see GL46::glBeginQueryIndexed} and
     * {@see GL46::glEndQueryIndexed}, the counter will not be incremented.
     * When {@see GL46::glEndQueryIndexed} is executed, the
     ...
     * {@see GL46::glGetQueryObject} with $pname
     * {@see GL46::GL_QUERY_RESULT}. When $target is
     * {@see GL46::GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN}, $index must be
     * less than the value of {@see GL46::GL_MAX_VERTEX_STREAMS}.
     *
     * If $target is {@see GL46::GL_TIME_ELAPSED}, $id must be an unused
     * name, or the name of an existing timer query object previously bound
     * to the {@see GL46::GL_TIME_ELAPSED} query binding. When
     * {@see GL46::glBeginQueryIndexed} is executed, the query object's time
     * counter is reset to 0. When {@see GL46::glEndQueryIndexed} is
     * executed, the elapsed server time that has passed since the call to
     * {@see GL46::glBeginQueryIndexed} is written into the query object's
     * time counter. This value can be queried by calling
     * {@see GL46::glGetQueryObject} with $pname
     * {@see GL46::GL_QUERY_RESULT}. When $target is
     * {@see GL46::GL_TIME_ELAPSED}, $index must be zero.
     *
     * Querying the {@see GL46::GL_QUERY_RESULT} implicitly flushes the GL
     * pipeline until the rendering delimited by the query object has
     * completed and the result is available.
     * {@see GL46::GL_QUERY_RESULT_AVAILABLE} can be queried to determine if
     ...
     * @return void
     */
    public function glBeginQueryIndexed($target, $index, $id): void
    {
        $target = $target instanceof \FFI\CData ? $target->cdata : $target;
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
        $id = $id instanceof \FFI\CData ? $id->cdata : $id;
    ...
    /**
     * {@see GL46::glGetQueryIndexediv} returns in $params a selected
     * parameter of the indexed query object target specified by $target and
     * $index. $index specifies the index of the query object target and must
     * be between zero and a target-specific maxiumum.
     *
     ...
     * for the specified $index of $target, or zero if no query is active,
     * will be placed in $params. If $pname is
     * {@see GL46::GL_QUERY_COUNTER_BITS}, the implementation-dependent
     ...
     * @return void
     */
    public function glEndQueryIndexed($target, $index): void
    {
        $target = $target instanceof \FFI\CData ? $target->cdata : $target;
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
     ...
     * @return void
     */
    public function glGetQueryIndexediv($target, $index, $pname, ?\FFI\CData $params): void
    {
        $target = $target instanceof \FFI\CData ? $target->cdata : $target;
        $index = $index instanceof \FFI\CData ? $index->cdata : $index;
        $pname = $pname instanceof \FFI\CData ? $pname->cdata : $pname;
